...
**Status:** Production-Ready / completed. The temporary `stage` sandbox proof and real serving `stage` proof completed, and the operator later authorized `main`; this TODO is now archived with row-level evidence.
...
**Next exact step:** none; this stage-readiness lane is closed.
...
- [x] Leave the repositories and operational notes ready for a later `main` promotion:
...
- [x] Workflow/runtime assumptions needed for the later `main` promotion are documented and resolved.
...
|---|---|---|---|---|---|---|
| docker root environment invariants | Tuple, rollback, runtime mutation, SSH timeout, web-app workflow materialization, and deploy gate logic live in Docker orchestration scripts | `bash .github/scripts/verify_environment_ci.sh` | Stage archive | passed | `2026-05-24` local rerun returned `OK: CI environment invariants validated.` | Re-run after the final reconciliation before archiving |
| docker CI surface audit | The TODO changed Docker CI orchestration and must prove the declared Laravel, Flutter, and Docker surfaces are still visible | `bash delphi-ai/tools/ci_pipeline_surface_audit.sh --repo . --expect docker --expect flutter --expect laravel` | Stage archive | passed | Evidence recorded in this TODO as `Overall outcome: ready` after topology inspection fix | Advisory topology warnings were fixed before final lane proof |
| docker deploy and rollback scripts syntax | The deploy and rollback proof path is implemented in shell scripts executed by GitHub Actions | `bash -n .github/scripts/rollback_over_ssh.sh .github/scripts/rollback_remote.sh .github/scripts/deploy_stage_over_ssh.sh .github/scripts/mark_successful_revision_over_ssh.sh .github/scripts/capture_successful_release_tuple_over_ssh.sh .github/scripts/check_remote_web_runtime_sha_over_ssh.sh` | Stage archive | passed | `2026-05-24` shell syntax rerun exited `0` | Covers touched runtime deploy, rollback, tuple capture, and runtime SHA scripts |
| Flutter web navigation harness policy | The stage proof depends on Playwright browser navigation smoke results not being swallowed by the harness | `node --test tools/flutter/web_app_tests/navigation_harness_policy_test.cjs` | Stage archive | passed | Evidence recorded in this TODO as `pass 1 / fail 0` after `run_with_timeout` fix | Source-owned regression for timeout and non-zero propagation |
| Laravel public media CORS feature test | Stage browser readonly proof hit tenant branding assets through landlord pages, so Laravel public media CORS is in scope | `docker compose exec -T app bash -lc 'set -a && source .env.testing && set +a && php artisan test tests/Feature/Security/PublicMediaCorsTest.php'` | Stage archive | passed | Evidence recorded in this TODO as `9 passed (39 assertions)` | Confirms cross-origin landlord-to-tenant branding media contract |

## Completion Evidence Matrix

| Criterion ID | Source Section | Criterion | Evidence Type | Evidence Artifact / Command | Runtime Target | Status | Notes |
|---|---|---|---|---|---|---|---|
| SCOPE-01 | Scope | Implement the minimal release tuple contract needed for forward success, rollback targeting, and post-rollback proof. | Code and live lane proof | `.github/scripts/capture_successful_release_tuple_over_ssh.sh`, `.github/scripts/mark_successful_revision_over_ssh.sh`, `.github/scripts/check_remote_web_runtime_sha_over_ssh.sh`, stage run `26315736532`, public `https://belluga.app/build_metadata.json` | GitHub Actions `stage` lane and public web runtime | passed | Real tuple recorded root SHA, web runtime SHA, lane, and public web build metadata for stage |
| SCOPE-02 | Scope | Implement the forward success-marker gate so unexpected `initialize=403` can no longer bypass proof on previously healthy lanes. | Code and local guard | `.github/scripts/deploy_stage_over_ssh.sh`, `.github/scripts/verify_environment_ci.sh`, local command `bash .github/scripts/verify_environment_ci.sh` | Docker deploy gate and stage runtime initialization path | passed | Current guard returned `OK: CI environment invariants validated.` and the TODO records the fail-closed initialize handling |
| SCOPE-03 | Scope | Implement the shared rollback proof / degraded-state handling contract so rollback result quality is explicit and fail-closed. | Code and destructive lane proof | `.github/scripts/rollback_over_ssh.sh`, `.github/scripts/rollback_remote.sh`, `.github/workflows/orchestration-ci-cd.yml`, stage run `26124523256` attempt `2` | Temporary `stage` proof host `201.54.9.139` and web browser navigation smoke | passed | External rollback ran after post-mutation failure, rollback proof passed, restored readonly and mutation navigation smoke passed, then job failed closed intentionally |
| SCOPE-04 | Scope | Validate locally everything that is logic/contract-shaped before touching `stage`. | Local CI-equivalent matrix | `bash .github/scripts/verify_environment_ci.sh`, `bash -n ...`, YAML parse evidence, `node --test tools/flutter/web_app_tests/navigation_harness_policy_test.cjs`, source-owned spec `tools/flutter/web_app_tests/navigation_harness_policy_test.cjs`, canonical runner `tools/flutter/run_web_navigation_smoke.sh readonly`, canonical runner `tools/flutter/run_web_navigation_smoke.sh mutation`, build proof `scripts/build_web.sh ../web-app stage`, public runtime provenance `__WEB_BUILD_SHA__=135a13e9` | Local Docker, Flutter web harness, Laravel test containers, and stage browser runtime | passed | Logic and contract checks were rerun after material script and workflow changes, with Playwright browser navigation smoke and web build provenance tied to stage |
| SCOPE-05 | Scope | Define the smallest honest first `stage` proof subset to run, then expand to the full applicable matrix. | Execution record | Stage runs `26046088556`, `26053825498`, `26060547111`, `26063868138`, `26066865339`, `26124523256`, `26311555749`, `26315736532` | Temporary stage host, then real stage lane public web runtime | passed | The file records progressive subset expansion from deploy/provenance to readonly, mutation, rollback, and real public domain proof |
| SCOPE-06 | Scope | Execute the real promotion path required to land the bugfix on `stage`. | GitHub lane evidence | Docker PR `#748` `dev -> stage`, stage run `26315736532`, completion guard `github_promotion_completion_guard.sh --lane stage --scenario flutter-only` | GitHub promotion lane and public stage web runtime | passed | `origin/stage` advanced to `21dbb27bea4c68be4d4ea94f31f6754d6e5dd668` and guard returned `Overall outcome: go` |
| SCOPE-07 | Scope | Exercise the real deploy/runtime-mutation matrix rows on `stage` only after the current local change set is coherent. | Live GitHub Actions proof | Stage run `26124523256` attempt `1` happy path and attempt `2` post-mutation rollback path | Temporary `stage` proof host with Playwright readonly and mutation navigation smoke | passed | Attempt 1 proved deploy, provenance, readonly, mutation, and success marking; attempt 2 proved runtime mutation failure, external rollback, rollback proof, and restored mutation navigation smoke |
| SCOPE-08 | Scope | Finish with `stage` serving the bugfix version in a healthy state. | Public runtime proof | `https://belluga.app/build_metadata.json`, `https://guarappari.belluga.app/build_metadata.json`, stage run `26315736532` successful-release marking | Real public stage web runtime | passed | Public metadata showed flutter SHA `135a13e9`, source branch `stage`, and `window.__LANDLORD_HOST__ = "belluga.app"` |
| SCOPE-09 | Scope | Leave the repositories and operational notes ready for a later `main` promotion: | Operational closure | This TODO records no missing workflow assumptions, no sequencing ambiguity, and no stale matrix evidence; later user authorized main after this lane | Docker, Laravel, Flutter promotion lanes | passed | This is the parent line for the operational-note subrequirements in the Scope section |
| DOD-01 | Definition of Done | The local logic/contract slices in scope are implemented and validated. | Local CI-equivalent matrix | `bash .github/scripts/verify_environment_ci.sh`, `bash -n ...`, `node --test tools/flutter/web_app_tests/navigation_harness_policy_test.cjs`, source-owned spec `tools/flutter/web_app_tests/navigation_harness_policy_test.cjs`, canonical runner `tools/flutter/run_web_navigation_smoke.sh readonly`, canonical runner `tools/flutter/run_web_navigation_smoke.sh mutation`, build proof `scripts/build_web.sh ../web-app stage`, public runtime provenance `__WEB_BUILD_SHA__=135a13e9`, Laravel `PublicMediaCorsTest.php` | Local Docker, Flutter web harness, Laravel test runtime, and stage browser runtime | passed | Every local suite row above is passed with command evidence, and browser-visible navigation evidence is tied to the stage web build |
| DOD-02 | Definition of Done | The real `dev -> stage` path needed for this bugfix has been executed. | GitHub lane evidence | Bot sync PRs `#746`, `#747`, Docker `dev -> stage` PR `#748`, run `26315736532` | GitHub promotion lane and public stage web runtime | passed | The stage path used lane-owned bot sync and Docker promotion, not a manual side channel |
| DOD-03 | Definition of Done | The fail-closed matrix rows that require real deploy/runtime mutation have been exercised on `stage` with recorded evidence. | Destructive runtime proof | Stage run `26124523256` attempt `2` | Temporary stage proof host, rollback executor, rollback proof, Playwright readonly and mutation navigation smoke | passed | Post-mutation SSH failure triggered external rollback and proof; restored readonly and mutation browser navigation smoke passed before intentional fail-closed job end |
| DOD-04 | Definition of Done | Forward proof, rollback proof, and degraded/incident handling behave consistently with the governing matrix on the real proof field. | Live lane matrix proof | Stage run `26124523256` attempts `1` and `2`, real run `26315736532`, `github_promotion_completion_guard.sh --lane stage --scenario flutter-only`, `tools/flutter/run_web_navigation_smoke.sh readonly`, `tools/flutter/run_web_navigation_smoke.sh mutation` | Temporary stage proof host and real public stage web runtime with Playwright browser navigation smoke | passed | Happy path marked success; destructive path rolled back and failed closed; readonly and mutation browser navigation smoke passed; real stage promotion guard returned `Overall outcome: go` |
| DOD-05 | Definition of Done | `stage` ends healthy on the bugfix version. | Public runtime proof | `https://belluga.app/build_metadata.json`, `https://guarappari.belluga.app/build_metadata.json`, stage run `26315736532` | Real public stage web runtime | passed | Real stage served the bugfix web build and public runtime probes passed |
| DOD-06 | Definition of Done | Workflow/runtime assumptions needed for the later `main` promotion are documented and resolved. | Documentation and code cross-check | This TODO plus `.github/workflows/orchestration-ci-cd.yml`, `.github/scripts/verify_environment_ci.sh`, `.github/scripts/rollback_over_ssh.sh` | Docker promotion workflows and rollback runtime | passed | Source-branch provenance and web-app-derived runtime assumptions were later handled by follow-up main-promotion TODOs without reopening this stage proof |
| DOD-07 | Definition of Done | All claimed-passing matrix evidence was produced after the latest material CI/deploy/rollback change in scope; no stale pre-change pass is being carried forward. | Evidence chronology | Latest local invariant rerun `2026-05-24`, real stage run `26315736532`, earlier stale rows explicitly invalidated in this TODO | Local CI-equivalent and GitHub stage runtime | passed | The TODO records when earlier evidence became stale and replaces it with later green runs on the current candidate generation |
| VAL-01 | Validation Steps | `bash delphi-ai/tools/ci_pipeline_surface_audit.sh --repo . --expect docker --expect flutter --expect laravel` | Command evidence | `bash delphi-ai/tools/ci_pipeline_surface_audit.sh --repo . --expect docker --expect flutter --expect laravel` | Local CI surface audit | passed | Recorded in the TODO as `Overall outcome: ready` after topology inspection fix |
| VAL-02 | Validation Steps | `bash .github/scripts/verify_environment_ci.sh` | Command evidence | `bash .github/scripts/verify_environment_ci.sh` | Local Docker orchestration guard | passed | `2026-05-24` rerun returned `OK: CI environment invariants validated.` |
| VAL-03 | Validation Steps | Local/static validation of the tuple / gate / rollback-proof logic changes. | Command evidence | `bash -n .github/scripts/rollback_over_ssh.sh .github/scripts/rollback_remote.sh .github/scripts/deploy_stage_over_ssh.sh .github/scripts/mark_successful_revision_over_ssh.sh .github/scripts/capture_successful_release_tuple_over_ssh.sh .github/scripts/check_remote_web_runtime_sha_over_ssh.sh`, paired with `tools/flutter/run_web_navigation_smoke.sh readonly` and `tools/flutter/run_web_navigation_smoke.sh mutation` stage rows | Local shell syntax, Docker script surface, and stage Playwright browser navigation smoke | passed | `2026-05-24` syntax rerun exited `0`; runtime-facing effects are covered by the stage browser navigation smoke rows |
| VAL-04 | Validation Steps | `node --test tools/flutter/web_app_tests/navigation_harness_policy_test.cjs` | Command evidence | `node --test tools/flutter/web_app_tests/navigation_harness_policy_test.cjs`, source-owned spec `tools/flutter/web_app_tests/navigation_harness_policy_test.cjs`, canonical runner `tools/flutter/run_web_navigation_smoke.sh readonly`, canonical runner `tools/flutter/run_web_navigation_smoke.sh mutation`, build proof `scripts/build_web.sh ../web-app stage`, public runtime provenance `__WEB_BUILD_SHA__=135a13e9` | Local Flutter web Playwright harness policy and real stage browser runtime | passed | Recorded as `pass 1 / fail 0`; protects browser navigation smoke status propagation and is tied to stage web build provenance |
| VAL-05 | Validation Steps | Evidence of the real lane-owned promotion flow for this bugfix, including the `dev -> stage` path and the `bot/next-version` sync only if/when that step is naturally exercised by the flow. | GitHub lane evidence | Bot sync PRs `#746`, `#747`, Docker `dev -> stage` PR `#748`, stage run `26315736532` | GitHub promotion lane and public stage web runtime | passed | Bot sync was exercised by the lane-owned flow and real stage run passed deploy, public edge, provenance, readonly navigation smoke, mutation navigation smoke, and success marking |
| VAL-06 | Validation Steps | `stage` evidence for the targeted real deploy/runtime-mutation matrix rows. | Runtime and browser evidence | Stage run `26124523256` attempt `2` and run `26315736532` | Temporary stage proof host plus real public stage web runtime | passed | Destructive runtime mutation row passed rollback proof and restored browser navigation smoke; real stage row passed public runtime proof |
| VAL-07 | Validation Steps | After each material workflow/script/runtime change, rerun the full applicable matrix subset and replace stale prior evidence instead of appending only new downstream rows. | Evidence chronology | This TODO marks stale rows after material changes and records later reruns `26124523256`, `26311555749`, `26315736532`, plus local `2026-05-24` invariant rerun | Local CI-equivalent and GitHub stage runtime | passed | No stale pre-change pass is used as the final delivery claim |
