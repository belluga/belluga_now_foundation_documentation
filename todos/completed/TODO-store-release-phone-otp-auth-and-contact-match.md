# TODO (Store Release): Phone OTP Auth And Contact Match

**Status legend:** canonical PACED delivery stages; the authoritative current stage is recorded in `Delivery Status Canon`.
**Status:** Production-Ready. Historical archival catch-up on `2026-06-08` confirmed that current `origin/main` still carries the tenant-public phone-OTP auth cutover, the Flutter/Laravel/browser/device closure anchors, and the supporting module documentation for this slice.
...
- **Approval scope:** documentation-only archival closeout for this bounded phone-OTP auth and contact-match slice after confirming the delivered contract still exists on current `origin/main`.
...
| `ARCH-OTP-01` | `Main Promotion Evidence - 2026-06-08 (Historical Archival Catch-Up)` | Current `origin/main` still carries the Flutter phone-OTP public-auth cutover, visible SMS fallback affordance, and auto-verify/error-handling coverage. | `origin/main review` | `git -C flutter-app grep -n "AuthPhoneOtpExperience\\|Receber por SMS\\|tenant public otp experience replaces the legacy login page structure\\|otp experience verifies automatically once when code is completed\\|Código incorreto" origin/main -- lib/presentation/shared/auth/screens/auth_login_screen test/presentation/common/auth/screens/auth_login_screen/auth_login_controller_contract_test.dart` | `origin/main source history` | `passed` | The launch public-auth UI and its focused regression anchors remain present on current `origin/main`. |
| `ARCH-OTP-02` | `Main Promotion Evidence - 2026-06-08 (Historical Archival Catch-Up)` | Current `origin/main` still carries the Laravel OTP challenge/verify, contact-hash materialization, queue/webhook, and public-environment redaction contracts. | `origin/main review` | `git -C laravel-app grep -n "test_phone_otp_challenge_explicit_sms_uses_secondary_otp_webhook\\|test_phone_otp_verification_promotes_identity_and_materializes_contact_match_hash\\|test_environment_api_exposes_phone_otp_sms_fallback_flag_without_webhook_url" origin/main -- tests/Feature/Auth/TenantPhoneOtpAuthTest.php tests/Api/v1/Tenants/Branding/ApiV1EnvironmentApiTest.php` | `origin/main source history` | `passed` | The backend auth and environment contracts consumed by Flutter remain present on current `origin/main`. |
| `ARCH-OTP-03` | `Canonical Module Anchors` | Canonical docs on `origin/main` still encode phone-OTP-only authenticated upgrade, post-auth hydration, and invite/history preservation rules. | `doc review` | `git -C foundation_documentation grep -n "Authenticated app upgrade is phone-OTP only\\|phone_otp\\|post-auth hydration\\|OTP verification must send the current anonymous identity id" origin/main -- modules/flutter_client_experience_module.md modules/onboarding_flow_module.md modules/invite_and_social_loop_module.md` | `foundation origin/main docs` | `passed` | The durable auth/merge/hydration contract is not stranded only inside the tactical TODO. |
| `ARCH-OTP-04` | `Approval` | This archival move is an explicit documentation closeout request, not a fresh implementation or promotion claim. | `approval` | Explicit `2026-06-08` user request plus `promotion-lane-code-main-audit-20260608.md` | `historical archival closeout` | `passed` | The closeout is intentional and traceable. |
...
| --- | --- | --- | --- | --- | --- |
| WhatsApp/SMS OTP delivery provider | OTP dispatch cannot ship without an approved outbound provider path and fallback channel behavior. | `healthy` | `2026-04-30` | `focused Laravel contract/tests plus recorded runtime/manual validation on the configured production-like Belluga tenant` | The delivered Belluga tenant path is validated for the recorded release packet; future provider/template drift must reopen a new TODO rather than keep this archival slice in promotion lane. |
...
- [x] Confirm live receiving URLs/provider/template approval through the recorded `2026-04-30` runtime/manual validation on the configured production-like Belluga tenant. Any future provider/template drift is a new operational follow-up, not an open blocker on this archival closeout.
...
| --- | --- | --- | --- | --- | --- | --- |
| `Historical archival catch-up` | No new code was executed for this move; the TODO already contains criterion-specific Flutter/Laravel/Playwright/device evidence and the archival action only reconciles lane status with current `origin/main`. | `n/a` | `historical archival closeout` | `n/a` | Existing `Current Execution Evidence`, `Latest Automated Evidence (2026-04-30)`, and `origin/main` contract review on `2026-06-08`. | Documentation-only move; no fresh CI-equivalent rerun was required. |

## Main Promotion Evidence - 2026-06-08 (Historical Archival Catch-Up)
| Surface | Evidence | Final SHA / Run |
| --- | --- | --- |
| `flutter-app` contract review | `git -C flutter-app grep -n "AuthPhoneOtpExperience\\|Receber por SMS\\|tenant public otp experience replaces the legacy login page structure\\|otp experience verifies automatically once when code is completed\\|Código incorreto" origin/main -- lib/presentation/shared/auth/screens/auth_login_screen test/presentation/common/auth/screens/auth_login_screen/auth_login_controller_contract_test.dart` | `origin/main` still carries the tenant-public OTP surface and focused regression coverage. |
| `laravel-app` contract review | `git -C laravel-app grep -n "test_phone_otp_challenge_explicit_sms_uses_secondary_otp_webhook\\|test_phone_otp_verification_promotes_identity_and_materializes_contact_match_hash\\|test_environment_api_exposes_phone_otp_sms_fallback_flag_without_webhook_url" origin/main -- tests/Feature/Auth/TenantPhoneOtpAuthTest.php tests/Api/v1/Tenants/Branding/ApiV1EnvironmentApiTest.php` | `origin/main` still carries the backend OTP challenge/verify/hash/environment contracts. |
| `foundation_documentation` doc review | `git -C foundation_documentation grep -n "Authenticated app upgrade is phone-OTP only\\|phone_otp\\|post-auth hydration\\|OTP verification must send the current anonymous identity id" origin/main -- modules/flutter_client_experience_module.md modules/onboarding_flow_module.md modules/invite_and_social_loop_module.md` | Canonical docs on `origin/main` still encode the auth cutover, merge, and hydration rules. |
| `foundation_documentation` history review | `git -C foundation_documentation log --oneline --decorate=no origin/main --grep="docs(otp): record browser runtime closure\\|docs(otp): consolidate post-auth hydration contract\\|docs(otp): close production error handling blocker" -n 10` | `origin/main` history still carries the docs-side closure anchors for runtime/browser/error-handling follow-through. |
...
| --- | --- | --- | --- | --- | --- |
| `Historical archival catch-up` | No new promotion PR package is being opened; confirm this move only reconciles a stale TODO with code and docs already present on `origin/main`. | `n/a` | `git -C flutter-app grep -n "Receber por SMS\\|otp experience verifies automatically once when code is completed" origin/main -- test/presentation/common/auth/screens/auth_login_screen/auth_login_controller_contract_test.dart`; `git -C laravel-app grep -n "test_phone_otp_verification_promotes_identity_and_materializes_contact_match_hash" origin/main -- tests/Feature/Auth/TenantPhoneOtpAuthTest.php` | `none` | No fresh PR/Copilot surface exists for this documentation-only move. |

## Rule-Spirit Anti-Pattern Hunt
| Rule / Principle Surface | Bypass or Anti-Pattern Search Lens | Status | Evidence Artifact / Command | Findings | Resolution / Notes |
| --- | --- | --- | --- | --- | --- |
| `Promotion-lane archive hygiene` | Prevent a code-absorbed, browser/device-validated phone-OTP auth slice from lingering in `promotion_lane/` only because the older TODO never received final closeout normalization. | `passed` | `origin/main` contract review; `promotion-lane-code-main-audit-20260608.md` | `no findings` | The closeout preserves the original Flutter/Laravel/browser/device packet and only reconciles stale lane status. |
...
- **Completed path:** `foundation_documentation/todos/completed/TODO-store-release-phone-otp-auth-and-contact-match.md`
- **Closeout decision:** archival catch-up approved on `2026-06-08` after confirming the phone-OTP auth and contact-match contract remains present on current `origin/main`.
- **Historical note:** this TODO already carried the required Flutter/Laravel/browser/device packet and later hydration/error-handling/runtime closure notes; the archival move only reconciles stale lane status.
- **Reopen rule:** any new tenant-public phone-OTP auth regression, environment exposure leak, or contact-match identity drift must open a new TODO.
