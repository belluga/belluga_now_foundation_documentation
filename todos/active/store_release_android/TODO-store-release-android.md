...
- [x] Treat invite share-code session context as part of the current occurrence migration blocker so preview -> event detail can preserve pending invite context before explicit acceptance.
...
| Invite occurrence target migration | `pending` | `pending` | `pending` | `pending` | `Local-Implemented-Playwright-Validated; Wave 2 share-code session-context addendum has focused Flutter/analyzer/web-build evidence plus source-owned Playwright mutation runtime proof` |
...
- [x] Post-release hardening TODOs that are not blockers for the current Android gate are separated under `active/post_release_hardening/`.
...
- [x] Post-release hardening TODOs are discoverable under `active/post_release_hardening/`.
