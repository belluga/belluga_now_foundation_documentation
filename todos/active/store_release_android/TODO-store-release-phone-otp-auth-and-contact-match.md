...
- **Current delivery stage:** `Local-Implemented-Functional-UX-Redesign-Pending`
- **Qualifiers:** `Business-Core`, `Cross-Stack`, `Release-Critical`, `Upstream-Baseline-Ready`, `Product-UX-Gap-Reopened`, `Stitch-Design-Required`
- **Next exact step:** generate and approve a Stitch-backed modern phone/OTP layout direction, then replace the current phone-entry and OTP-entry layout while preserving the functional OTP behavior, admin settings model, backend channel contract, and existing non-visual tests.
...
- [ ] Model tenant-admin OTP delivery settings as one primary WhatsApp webhook plus one optional SMS secondary channel; do not expose a generic `Webhook OTP`, a `Use WhatsApp webhook for OTP` switch, or an `OTP Channel` dropdown in the release UI.
- [ ] Let tenant-public OTP challenge default to WhatsApp and expose SMS only as an explicit secondary "other ways" send option when SMS is configured.
- [ ] Replace the legacy login-derived phone/OTP UI with a release-grade OTP flow: country picker, country-aware phone mask/validation, separated OTP character boxes, paste-to-fill behavior, resend/cooldown states, and clear channel affordances.
...
- [ ] Tenant-admin technical integrations expose the release product model for outbound OTP only: `Webhook WhatsApp`, `Secondary OTP Channel com SMS`, and conditional `URL SMS`.
- [ ] OTP secondary-channel behavior is explicit end-to-end: WhatsApp is the default challenge channel, SMS is available only when configured, and public UI surfaces `Outras formas` only for that configured fallback.
- [ ] Playwright source-owned web evidence covers every web-visible/admin-critical row in the matrix below; Android-only OTP UI closure remains ADB/device evidence in the final consolidated device phase.
...
- [ ] Tenant-admin technical integrations expose simplified `outbound_integrations` WhatsApp/SMS OTP settings and save/read back the derived backend settings namespace without leaking legacy `Webhook OTP`, `Use WhatsApp webhook for OTP`, or `OTP Channel` controls.
- [ ] Playwright matrix in this TODO is implemented under `tools/flutter/web_app_tests/**`, executed only through `tools/flutter/run_web_navigation_smoke.sh readonly|mutation`, and recorded as browser-stage evidence after the current web bundle is published.
...
- `tools/flutter/web_app_tests/otp_auth_public.spec.js` covers phone-first public OTP, WhatsApp default, SMS fallback, and routed challenge calls without real webhook dispatch.
- `tools/flutter/web_app_tests/navigation_mutation_shards.json` now includes deterministic shard `otp-auth`.
- `node --check` passed for both OTP specs; `NODE_PATH="$PWD/node_modules" npx playwright test --config ./playwright.config.js --grep '@mutation.*OTP Auth' --list --reporter=line` plus shard validation selected exactly the two expected tests.

### Reopened Visual Redesign Gap (2026-04-29)

- **User QA result:** the current phone-entry and OTP-entry implementation is functionally acceptable but visually unacceptable for release. It remains too close to the legacy login pattern and does not satisfy the request for a fully reformulated modern OTP experience.
...
- **Visual baseline to replace:** the existing login-derived phone/OTP layout and current code/phone screens may be discarded entirely.
- **Design requirement:** use Stitch as an explicit design input before implementation. Save/record generated layout references in this TODO or a supporting artifact before coding the replacement UI.
- **Execution boundary:** this is a visual/layout redesign of tenant-public OTP auth. It must not reopen admin outbound settings, OTP backend delivery, contact-hash matching, anonymous merge, or authenticated web scope.

### Reopened Visual Task Matrix
| Task ID | Task | Primary Owner Surface | Required Evidence Before Closure |
| --- | --- | --- | --- |
| `T2R-FL-04` | Generate modern phone-entry and OTP-entry layout proposals with Stitch, then select the release direction. | Stitch design workflow + Flutter tenant-public auth surface. | Saved Stitch screen references or exported images; TODO decision note selecting the direction. |
| `T2R-FL-05` | Replace the current phone-entry layout with the approved modern layout while preserving country-aware phone behavior. | Flutter tenant-public auth presentation. | Widget/controller tests proving phone behavior still passes; screenshot/browser/device evidence for visual layout. |
| `T2R-FL-06` | Replace the current OTP-entry layout with the approved modern layout while preserving segmented input, paste, resend/cooldown, channel context, and errors. | Flutter tenant-public auth presentation. | Widget/controller tests plus visual/runtime evidence for OTP state variants. |
...
| --- | --- | --- | --- | --- |
| `T2R-FL-04` Stitch design selection | Implementation starts without approved visual target. | n/a | Saved Stitch references/images plus selected-direction note. | `planned` |
| `T2R-FL-05` phone-entry replacement | Country selection, mask, validation, E.164 payload, WhatsApp default CTA. | Flutter widget/controller/repository tests for phone challenge. | Screenshot/browser if renderable; final ADB for mobile layout. | `planned` |
| `T2R-FL-06` OTP-entry replacement | Segmented input, paste-to-fill, verify validation, errors, resend/cooldown, SMS fallback. | Flutter widget/controller tests for OTP state variants and channel actions. | Screenshot/browser if renderable; final ADB for keyboard/small-screen layout. | `planned` |
| `T2R-FL-06` race/disabled states | Double send/resend/verify creates duplicate challenge or incoherent state. | Frontend race-condition validation + controller tests. | Browser/device runtime when env/device is available. | `planned` |
...
- **Public OTP visual release gap reopened:** user QA on 2026-04-29 rejected the current visual/layout direction as too close to the legacy login flow. Functional behavior stays; the phone/OTP layout must be replaced by a Stitch-backed modern OTP design before release closure.
...
- **Validation evidence:** `fvm flutter test test/presentation/common/auth/screens/auth_login_screen/auth_login_controller_contract_test.dart test/infrastructure/repositories/auth_repository_signup_test.dart test/presentation/tenant_admin/settings/tenant_admin_settings_screen_test.dart test/infrastructure/repositories/tenant_admin_settings_repository_test.dart` passed with `88/88` tests after the ValueObject/analyzer fix.
...
- **Playwright matrix evidence:** source-owned OTP specs and shard `otp-auth` were added; `node --check` passed and Playwright list/shard validation selected exactly the two OTP Auth tests. Full runtime execution is pending environment variables `NAV_TENANT_URL`, `NAV_ADMIN_EMAIL`, and `NAV_ADMIN_PASSWORD`.
