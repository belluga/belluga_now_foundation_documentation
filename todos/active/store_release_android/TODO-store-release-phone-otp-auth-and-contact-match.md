...
| Flutter auth cutover to phone OTP | `local` | `pending` | `pending` | `pending` | `Local-Implemented; release-grade country/masked phone entry, segmented OTP entry, and secondary SMS channel affordance covered by focused tests/source Playwright; ADB smoke deferred` |
...
| Tenant-admin outbound webhook configuration surface | `local` | `pending` | `pending` | `pending` | `Local-Implemented; simplified WhatsApp webhook + optional SMS secondary URL covered by focused tests/source Playwright; runtime Playwright shard still pending credentials/env` |
...
| `SCOPE-06` | Scope | Model tenant-admin OTP delivery settings as one primary WhatsApp webhook plus one optional SMS secondary channel; do not expose a generic `Webhook OTP`, a `Use WhatsApp webhook for OTP` switch, or an `OTP Channel` dropdown in the release UI. | Flutter admin UI and Playwright source | `test/presentation/tenant_admin/settings/tenant_admin_settings_screen_test.dart`; `tools/flutter/web_app_tests/otp_auth_admin.mutation.spec.js`; `scripts/build_web.sh ../web-app dev` | Local Flutter widget tests; Playwright mutation source/shard; refreshed `../web-app` bundle | passed | Admin surface exposes `Webhook WhatsApp`, `Secondary OTP Channel com SMS`, and conditional `URL SMS`. |
| `SCOPE-07` | Scope | Let tenant-public OTP challenge default to WhatsApp and expose SMS only as an explicit secondary "other ways" send option when SMS is configured. | Flutter controller/UI and Playwright source | `fvm flutter test test/presentation/common/auth/screens/auth_login_screen/auth_login_controller_contract_test.dart`; `tools/flutter/web_app_tests/otp_auth_public.spec.js` | Local Flutter widget tests; Playwright mutation source/shard | passed | Default request is WhatsApp; SMS is hidden behind `Outras formas` only when SMS fallback exists. |
...
| `DOD-03` | Definition of Done | Tenant-admin technical integrations expose the release product model for outbound OTP only: `Webhook WhatsApp`, `Secondary OTP Channel com SMS`, and conditional `URL SMS`. | Flutter admin tests and Playwright source | `fvm flutter test test/presentation/tenant_admin/settings/tenant_admin_settings_screen_test.dart test/infrastructure/repositories/tenant_admin_settings_repository_test.dart`; `tools/flutter/web_app_tests/otp_auth_admin.mutation.spec.js`; `tools/flutter/run_web_navigation_smoke.sh mutation` env gate recorded | Local Flutter tests; Playwright mutation source/shard; refreshed `../web-app` bundle | passed | Legacy controls are absent from widget/source Playwright assertions. |
| `DOD-04` | Definition of Done | OTP secondary-channel behavior is explicit end-to-end: WhatsApp is the default challenge channel, SMS is available only when configured, and public UI surfaces `Outras formas` only for that configured fallback. | Flutter controller/UI tests and Playwright source | `fvm flutter test test/presentation/common/auth/screens/auth_login_screen/auth_login_controller_contract_test.dart`; `tools/flutter/web_app_tests/otp_auth_public.spec.js` | Local Flutter tests; Playwright mutation source/shard | passed | Test covers WhatsApp default, hidden SMS label before popup, and explicit SMS challenge. |
| `DOD-05` | Definition of Done | Playwright source-owned web evidence covers every web-visible/admin-critical row in the matrix below; Android-only OTP UI closure remains ADB/device evidence in the final consolidated device phase. | Playwright source/list validation and approved runtime waiver | `node --check tools/flutter/web_app_tests/otp_auth_public.spec.js`; `node --check tools/flutter/web_app_tests/otp_auth_admin.mutation.spec.js`; `tools/flutter/run_web_navigation_smoke.sh mutation` hard-blocked by missing `NAV_ADMIN_EMAIL`/`NAV_ADMIN_PASSWORD`; `scripts/build_web.sh ../web-app dev` | Playwright source/shard; refreshed `../web-app` bundle; Android device `192.168.15.9:5555` | waived | Waiver approval: user `APROVADO` on 2026-04-29 authorized continuing when unavailable runtime gates are explicitly recorded; official mutation runner requires credentials not present in the environment. |
...
| `VAL-06` | Validation Steps | Flutter tests plus applicable browser/device runtime evidence cover the redesigned phone-entry/OTP validation errors, backend-error readability, loading/disabled CTA behavior, keyboard-safe small-width layout, country mask behavior, separated OTP boxes, paste-to-fill, resend/cooldown state, and secondary SMS affordance. | Flutter tests, analyzer, web build, Playwright source, ADB smoke | `fvm flutter test test/presentation/common/auth/screens/auth_login_screen/auth_login_controller_contract_test.dart`; `fvm dart analyze --format machine`; `scripts/build_web.sh ../web-app dev`; `tools/flutter/web_app_tests/otp_auth_public.spec.js`; `tools/flutter/run_web_navigation_smoke.sh mutation` env gate; direct `fvm flutter drive ... feature_auth_login_navigates_to_intended_route_test.dart -d 192.168.15.9:5555` | Local Flutter tests; browser Playwright source/list; refreshed `../web-app` bundle; Android device `192.168.15.9:5555` | passed | Widget coverage includes country-aware phone field, segmented OTP, full six-digit fill, backend error surface, disabled/loading CTA, and SMS fallback affordance. |
...
| `VAL-08` | Validation Steps | Tenant-admin technical integrations expose simplified `outbound_integrations` WhatsApp/SMS OTP settings and save/read back the derived backend settings namespace without leaking legacy `Webhook OTP`, `Use WhatsApp webhook for OTP`, or `OTP Channel` controls. | Flutter admin repository/widget tests and Playwright mutation source | `fvm flutter test test/presentation/tenant_admin/settings/tenant_admin_settings_screen_test.dart test/infrastructure/repositories/tenant_admin_settings_repository_test.dart`; `tools/flutter/web_app_tests/otp_auth_admin.mutation.spec.js`; `scripts/build_web.sh ../web-app dev`; `tools/flutter/run_web_navigation_smoke.sh mutation` env gate recorded | Local Flutter tests; Playwright mutation source/shard; refreshed `../web-app` bundle | passed | Repository payload keeps compatibility fields derived from the product model while rendered UI hides legacy controls. |
| `VAL-09` | Validation Steps | Playwright matrix in this TODO is implemented under `tools/flutter/web_app_tests/**`, executed only through `tools/flutter/run_web_navigation_smoke.sh readonly mutation`, and recorded as browser-stage evidence after the current web bundle is published. | Playwright source/list validation and approved runtime waiver | `node --check` for OTP specs; mutation shard list/validate selected two OTP Auth tests; readonly list selected `OTP-WEB-BOUNDARY-01`; `scripts/build_web.sh ../web-app dev`; `tools/flutter/run_web_navigation_smoke.sh mutation` hard-blocked by missing `NAV_ADMIN_EMAIL`/`NAV_ADMIN_PASSWORD` | Playwright source/shard; refreshed `../web-app` bundle | waived | Waiver approval: user `APROVADO` on 2026-04-29; browser runtime execution remains blocked until runtime credentials are present, while source/shard/build evidence is complete. |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `PW-OTP-ADM-01` | `T2R-ADM-01` | `tools/flutter/web_app_tests/otp_auth_admin.mutation.spec.js` / `@mutation OTP Auth admin exposes WhatsApp primary and optional SMS fallback without legacy controls` | `mutation` | Login through `support/tenant_admin_auth.js`; open tenant-admin technical integrations. | Inspect outbound webhook settings section. | Shows `Webhook WhatsApp` and `Secondary OTP Channel com SMS`; hides `Webhook OTP`, `Usar webhook WhatsApp para OTP`, and `Canal OTP`; no browser/runtime/console failures. | Source-created 2026-04-29; runtime execution still requires `NAV_TENANT_URL`, `NAV_ADMIN_EMAIL`, and `NAV_ADMIN_PASSWORD`. |
...
| `PW-OTP-WEB-BOUNDARY-01` | `D-06`, `D-13` | `tools/flutter/web_app_tests/otp_auth_public.spec.js` / `@readonly OTP-WEB-BOUNDARY-01` | `readonly` | Tenant URL only. | Open auth-owned tenant-public web paths such as `/auth/login` or a documented auth-gated web action. | Web remains app-promotion/handoff boundary and does not expose authenticated web OTP as a new capability. | This is a boundary guard, not mobile OTP UI proof. |
| `PW-OTP-PUBLIC-01` | `T2R-FL-01` | `tools/flutter/web_app_tests/otp_auth_public.spec.js` / `@mutation OTP Auth public requests WhatsApp by default and SMS fallback with segmented code UI` | `mutation` | Tenant validation target where the auth route renders the shared Flutter auth screen; environment response is routed to expose SMS secondary without changing live settings. | Open `/auth/login`; type a local BR phone number; submit. | Auth screen is phone-first, no email/password legacy controls; challenge request body is normalized to E.164 and carries explicit WhatsApp intent. | Runtime execution still requires `NAV_TENANT_URL`. |
| `PW-OTP-PUBLIC-02` | `T2R-FL-01`, `T2R-FL-02` | same spec / same mutation title | `mutation` | OTP challenge endpoint is routed to prevent real webhook dispatch during browser test. | Submit phone challenge once and wait for verification screen. | Verification screen shows channel/destination context, segmented-code UI path, and `Confirmar codigo`; no real provider is called. | Webhook dispatch itself closes through backend queue evidence. |
| `PW-OTP-PUBLIC-03` | `T2R-FL-02` | same spec / same mutation title plus Flutter widget/controller tests | `mutation` + Flutter focused | Verification screen reached. | Render OTP verification state and run controller tests. | Segmented `Pinput` widget is present in Flutter widget coverage; controller validates six digits and preserves channel context. | Browser paste-specific proof remains optional/ADB-final if web semantics cannot target the first OTP cell deterministically. |
| `PW-OTP-PUBLIC-04` | `T2R-FL-03` | same spec / same mutation title | `mutation` | SMS secondary configured by routed environment/settings payload. | Choose `Receber por SMS`. | SMS request carries `delivery_channel=sms`; UI names `SMS`; SMS affordance depends on configured secondary URL. | Backend missing-SMS rejection is covered by Laravel tests. |
| `PW-OTP-RACE-01` | `T2R-FL-01`, `T2R-FL-02` | same spec / `@mutation OTP-RACE-01` | `mutation` if browser-renderable; otherwise ADB final | Phone entry and verification screens. | Rapid double-tap send/resend/verify actions. | Buttons become disabled/in-flight deterministically; duplicate visible state is not created; cooldown/retry text remains consistent with backend response. | Pair with `frontend-race-condition-validation` during implementation. |
...
- `tools/flutter/web_app_tests/otp_auth_public.spec.js` covers tenant-public web auth-boundary readonly preservation, phone-first public OTP, WhatsApp default, SMS fallback, and routed challenge calls without real webhook dispatch.
- `tools/flutter/web_app_tests/navigation_mutation_shards.json` now includes deterministic shard `otp-auth`.
- `node --check` passed for both OTP specs; `NODE_PATH="$PWD/node_modules" npx playwright test --config ./playwright.config.js --grep '@mutation.*OTP Auth' --list --reporter=line` plus shard validation selected exactly the two expected mutation tests; `--grep '@readonly.*OTP-WEB-BOUNDARY' --list` selected `@readonly OTP-WEB-BOUNDARY-01 tenant-public web auth remains app promotion boundary`.
...
| `T2R-FL-06` race/disabled states | Double send/resend/verify creates duplicate challenge or incoherent state. | Frontend race-condition validation + controller tests. | Runtime mutation runner waived until credentials exist; controller/loading-state tests remain local evidence. | `waived` |
...
- **Playwright matrix evidence:** source-owned OTP specs and shard `otp-auth` were added; `node --check` passed, Playwright list/shard validation selected exactly the two OTP Auth mutation tests, and readonly listing selected `OTP-WEB-BOUNDARY-01`.
...
- **Runtime mutation follow-up (2026-04-30):** canonical `NAV_WEB_SHARD=otp-auth` no longer blocks on missing credentials, but it is not green yet. The admin test authenticated successfully and then exposed a runtime/admin settings gap: the technical integrations page rendered `FormApiFailure(status=404, message=Resource you are looking for was not found.)` while loading settings. The public mutation test currently conflicts with the release web boundary, because tenant-public `/auth/login` on web correctly renders app promotion instead of the in-app phone OTP form; phone/OTP UI behavior remains covered by Flutter tests and Android/device evidence, not by tenant-public web mutation.
