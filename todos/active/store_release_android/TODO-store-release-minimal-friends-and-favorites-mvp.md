...
- **Current delivery stage:** `Local-Implemented-Consumer-Gaps-External-Contacts-Round02-Adjudicated-ADB-Blocked-NoDevice`
...
- **Next exact step:** reconnect an ADB device so `adb devices` lists a target, then run the consolidated Wave 2D device smoke for contact permission, contact refresh, selected-occurrence share-code generation, native share-sheet handoff, Home favorite/unfavorite refresh, and occurrence-scoped presence/invite flows.
...
| Sharing CTA root cause | CTA moves from `Gerando...` to retry because share-code creation receives an invalid occurrence target. | Flutter application test proves event-to-invite factory sends selected `occurrence_id`, not event date/event fallback. | ADB final: generate a real share link for the selected occurrence and verify no retry state on valid backend settings. | `TODO-store-release-invites-occurrence-target-migration.md`; `local-passed / ADB-blocked-no-device` |
...
| `/convites/compartilhar` external share dispatch works locally | Widget tests `external phone contact share launches normalized WhatsApp target` and `external contact share falls back to system share` assert the tapped action dispatches a normalized `wa.me/5527...` URL, carries the invite URL payload, uses `LaunchMode.externalApplication`, and falls back to `SharePlus` parameters. | Passed 2026-04-29 |
...
| Flutter architecture/analyzer gate | `fvm dart analyze --format machine` | Passed 2026-04-29, no diagnostics after external-contact Round 01 fixes |
| Flutter web build gate | `bash scripts/build_web.sh ../web-app dev` | Passed 2026-04-29 after external-contact Round 01 fixes in 133.0s; `web-app` output is derived and not committed |
...
| Final ADB/device runtime proof | `adb devices` returned no attached device on 2026-04-29, so the consolidated device phase cannot start from this session. Required rows: contact permission, contact refresh, selected-occurrence share-code generation, native share-sheet handoff, Home favorite/unfavorite refresh, and occurrence-scoped presence/invite smoke. | Blocked / no ADB device |
...
| `/convites/compartilhar` + contact-group bulk invite convergence | `local` | `local-passed` | `ADB-blocked-no-device` | `pending` | `Local-Implemented; QA-reopened gaps have local root-cause fixes for share CTA selected occurrence, explicit refresh, import-match merge, OTP-compatible phone hashes, external-contact branch, normalized external share dispatch, and release payload cleanup; final ADB evidence is blocked until a device is attached` |
