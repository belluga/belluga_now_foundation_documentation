...
**Status:** Active. Account-profile favorites and contact-import invite targeting already exist; the reopened `/convites/compartilhar` consumer gaps have local fixes, round-01 and round-02 audit blockers are resolved, round-03 delta audit is adjudicated, and final device evidence remains deferred before promotion.
...
- **Current delivery stage:** `Local-Implemented-Consumer-Gaps-Round03-Adjudicated-ADB-Deferred`
- **Qualifiers:** `Business-Core`, `Cross-Stack`, `Release-Critical`, `Consumer-Gap-Reopened`, `Invite-Share-Regression`, `Refresh-Action-Required`, `Flutter-Consumer-Fix`, `Contact-Refresh-Root-Cause-Fixed-Local`, `Personal-Profile-Bootstrap-Fixed-Local`, `Bulk-Import-Fixed-Local`, `Widget-False-Green-Corrected`, `Round01-Audit-Resolved`, `Round02-Audit-Resolved`, `Round03-Audit-Adjudicated`
- **Next exact step:** consolidate the local checkpoint, then carry final ADB/device contact-permission, contact refresh, selected-occurrence share, and native share-sheet smoke to the consolidated ADB phase.
...
| `/convites/compartilhar` + contact-group bulk invite convergence | `local` | `local-passed` | `ADB-deferred` | `pending` | `Local-Implemented; QA-reopened gaps have local root-cause fixes for share CTA selected occurrence, explicit refresh, import-match merge, OTP-compatible phone hashes, and release payload cleanup; final ADB evidence remains deferred` |
