...
| --- | --- | --- | --- | --- |
| `round-01` | `foundation_documentation/artifacts/triple-audits/store-release-usability-quality-hardening-final-20260424/store-release-usability-quality-final-20260424T180850Z/package-triple-audit-20260424T180903Z` | `needs_adjudication` | Merge classified a recommended-path conflict because reviewers emphasized different remediation priorities, but the findings are compatible rather than contradictory: untracked deliverable risk, management query-shape risk, eager Flutter row construction, and static-only performance evidence. | Resolved in implementation by adding executable backend instrumentation tests, replacing the management query with a `$facet` page/count aggregate and page-bounded bulk load, removing eager chip prebuild in the horizontal filter row, and preparing the branch for tracked checkpoint before a fresh audit package. |
...
| `DOD-06` | `Definition of Done` | Home/Discovery click path proven by runtime navigation. | `runtime` | `bash tools/flutter/run_web_navigation_smoke.sh mutation` with final-domain env -> `19 passed (11.1m)` | `browser` | `passed` | Real chip clicks use semantic click with coordinate fallback only when needed, and assert backend filtered requests/visible results. |
| `DOD-07` | `Definition of Done` | Public filters do not leak invalid types/taxonomies. | `test+runtime` | Laravel catalog tests -> `2 passed (33 assertions)`; canonical Playwright mutation runner -> `19 passed` | `backend+browser` | `passed` | Discovery excludes non-favoritable profile types; Home Event taxonomies follow Event Type. |
...
| `DOD-12` | `Definition of Done` | Independent no-context triple audit clean against `dev...current`. | `review` | pending frozen comparison package + triple audit session | `cross-stack` | `pending` | Final quality gate; must use the exact branch state at audit launch. |
...
| `VAL-04` | `Validation Steps` | Flutter focused tests. | `test` | Combined focused Flutter suite -> `121 passed`; package rerun -> `12 passed` | `local` | `passed` | Widget/domain/repository/semantics coverage. |
| `VAL-05` | `Validation Steps` | Flutter analyzer. | `test` | `fvm dart analyze --format machine` -> exit `0` after replacing deprecated semantics API | `local` | `passed` | Official analyzer gate clean. |
| `VAL-06` | `Validation Steps` | Build web bundle. | `runtime` | `bash scripts/build_web.sh ../web-app dev` -> passed | `local->web-app` | `passed` | Built branch `08dea143`. |
| `VAL-07` | `Validation Steps` | Domain serves refreshed bundle. | `runtime` | local/remote `main.dart.js` SHA-256 `06eb539477c89aec32307f929bf7dccb130706d53fcd4fb8b213f1602f233696`; `__WEB_BUILD_SHA__=08dea143` | `browser` | `passed` | `https://guarappari.belluga.space` serves the current branch bundle. |
| `VAL-08` | `Validation Steps` | Playwright filter/Event runtime paths. | `runtime` | `NAV_LANDLORD_URL=https://belluga.space NAV_TENANT_URL=https://guarappari.belluga.space PLAYWRIGHT_IGNORE_HTTPS_ERRORS=true NAV_DEPLOY_LANE=dev NAV_WEB_TEST_TYPE=mutation NAV_WEB_WORKERS=1 bash tools/flutter/run_web_navigation_smoke.sh mutation` -> `19 passed (11.1m)` | `browser` | `passed` | Covers rich text, Map baseline, Home/Discovery filters, occurrence FAB/persistence, repeated public hydration, type taxonomy preload, and tenant agenda UI. |
