...
- **Approval scope:** create a narrow active TODO for browser/device/store/deferred closure plus one approved behavior refinement: tenant-public Home initial open must not auto-open the promotion modal, while the broader anonymous-web promotion boundary and hard/auth action gates remain unchanged.
...
What still lacks closure is the final runtime truth plus one narrow behavior refinement:

1. implement the approved Home exception so initial tenant-public Home access does not auto-open the promotion modal,
...
## Contract Boundary
- This TODO owns the residual Android runtime/store/deferred validation lane for the already-promoted web-to-app behavior plus one approved narrow behavior refinement: tenant-public Home initial open must not auto-open the promotion modal.
- The Home exception is not a relaxation of the anonymous-web boundary. Home remains read-only/showcase on first access; promotion still applies when the user explicitly hits a protected route or protected action from Home.
- It may drive small subordinate fixes needed to implement the Home exception or to resolve a concrete runtime defect, but it does not reopen the broader product boundary or redesign the conversion experience.
...
- **Qualifiers:** `Runtime-Validation`, `Android`, `Store`, `Deferred-Install`, `Home-Initial-Open-Exception`, `Split-From-Completed-2026-06-08`
- **Next exact step:** implement the approved Home initial-open exception, then run the real browser/device matrix against the current published Android target and capture evidence for Home first-access behavior, store destination, install referrer continuity, first-open restoration, and representative hard-gate entrypoints.

## Active Work State (Required While TODO Remains In `active/`)
- **Work state:** `implementation`
- **Why this state now:** implementation is materially present on `origin/main`, but this TODO now also owns one approved behavior change for Home initial access plus the still-open runtime closure lane.
...
## Scope
- [ ] `SCOPE-01` Implement the approved guard exception so tenant-public Home initial open does not auto-open the promotion modal.
...
## Out of Scope
- [ ] Broad reopening of the canonical web-to-app promotion policy or changing the already-promoted `/baixe-o-app` contract beyond the approved Home initial-open exception.
...
- **May stay inside this TODO:** the Home initial-open no-modal exception, browser/device validation, ADB/install checks, Play Store/deferred continuity evidence, and any small fix directly required to make the approved Android runtime behave as documented.
...
## Definition of Done
- [ ] `DOD-01` Tenant-public Home initial open no longer auto-opens the promotion modal and does not interrupt the first-access read-only experience.
...
- [ ] `VAL-02` Implement and locally verify the Home initial-open no-modal exception before final runtime capture.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `DOD-01` | `Definition of Done` | `DOD-01` Tenant-public Home initial open no longer auto-opens the promotion modal and does not interrupt the first-access read-only experience. | `runtime` | `<planned Home first-open browser/device evidence>` | `browser + Android device` | `planned` | This is the approved narrow exception to the current guard behavior. |
...
| `VAL-02` | `Validation Steps` | `VAL-02` Implement and locally verify the Home initial-open no-modal exception before final runtime capture. | `test+review` | `<planned focused Flutter/controller/web verification for Home guard exception>` | `local Flutter + browser` | `planned` | Ensures the new exception exists before final browser/device evidence. |
| `VAL-03` | `Validation Steps` | `VAL-03` Run the web/browser matrix for Home initial open, invite landing, public detail-route promotion, and guarded-route promotion. | `runtime` | `<planned Playwright/manual browser evidence>` | `browser` | `planned` | Must cover the new Home exception plus the existing representative promotion entrypoints. |
...
| --- | --- | --- | --- | --- |
| `operational-coder` | `assurance-tester-quality` | The code/policy slice is mostly absorbed, but this TODO now also carries one narrow Home-entry behavior change plus the residual runtime validation and defect classification lane. | `foundation_documentation/todos/active/v0.2.1+9/**`, published runtime targets, `flutter-app/**`, `laravel-app/**` if a bug is exposed | `planned` |
...
- [x] `D-05` Tenant-public Home initial open is the only approved exception to the current guard-triggered promotion interruption rule: first access stays read-only and uninterrupted, while explicit protected actions from Home still trigger the canonical promotion boundary.
...
- [x] `D-02` Tenant-public Home initial open is a narrow exception: first paint must remain uninterrupted by a promotion modal, but Home stays read-only and protected actions still promote the app.
...
| `A-03` | The Home initial-open interruption is currently caused by a local guard/promotion behavior that can be isolated without redesigning the wider anonymous-web boundary. | User direction on `2026-06-08` defines the exception as narrow and unique. | The TODO may need a broader policy/module update before safe implementation. | `Medium` | `Keep as Assumption` |
...
- `flutter-app/**` Home/public guard or promotion-entry surfaces needed to suppress the initial Home modal without relaxing protected-action gates
- published tenant/browser runtime targets used by the Android promotion flow
- Android device lane / ADB evidence surface
- `laravel-app/**` only if runtime validation exposes a concrete backend defect beyond the approved Home-entry exception

### Ordered Steps
1. Implement the approved Home initial-open no-modal exception without relaxing Home protected-action promotion behavior.
...
### Test Strategy
- **Strategy:** `test-after`
- **Why:** the remaining lane is still dominated by runtime/browser/device proof, even though one narrow Home-entry code change is now explicitly in scope.
- **Fail-first target(s) (when required):** `Home initial-open guard/promotion behavior tests if the current Flutter suite already freezes that path`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Home initial open without promotion modal | Visible first-access tenant-public behavior and explicit exception to the current guard interruption | `shared-android-web` | `both` | `no` | `yes` | `<planned browser + Android device evidence>` | `n/a` |
...
| --- | --- | --- | --- | --- | --- | --- |
| `flutter-app focused guard/promotion lane` | The approved Home-entry exception will require a concrete Flutter validation row once implementation starts. | `n/a at contract-authoring stage` | `Local-Implemented` | `n/a` | Current change only expands the TODO contract; exact Flutter CI-equivalent commands must be added before implementation is claimed. | No code implementation is being claimed in this documentation update. |
