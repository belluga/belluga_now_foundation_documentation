...
- **Qualifiers:** `Fast-Follow`, `Regression-Fix`, `Data-Integrity`, `Cross-Stack`, `Release-Confidence`, `Backend-Guarded`, `Runtime-Residual-Cleanup-Approved`, `Playwright-Cleanup-Hardened`
- **Next exact step:** run completion/closeout guards with the consolidated v0.2.0+8 package and keep promotion movement tied to the approved promotion lane.
...
- [x] Refine the repair command so known local test-seed account aggregates are deletable when the only linked-data blockers are soft-deleted event/occurrence references from harness cleanup, while active user, invite, active event, or active occurrence references still block deletion.
- [x] Clean all current local `guarappari` missing-profile residuals through the backend-owned command/service path; no invalid tenant-admin Account may remain visible after the repair.
- [x] Hard-delete repair-approved local/test harness account aggregates instead of leaving account/profile soft-delete residue, and provide a backend-owned local purge path for already-soft-deleted harness aggregates.
...
- **Last confirmed truth:** creation transaction still works; direct profile deletion is guarded; affected Playwright cleanup no longer deletes onboarding-created profiles directly; local corrupted test seeds were repaired through the backend command; backend-owned purge removed local soft-deleted harness residue; post-repair dry-run reports zero invalid accounts.
...
- [x] `VAL-07` GREEN: repair command/service execute repairs local `guarappari`, then dry-run reports zero invalid accounts.
...
- [x] `VAL-10` GREEN: post-validation backend invariant query proves no live local `guarappari` account lacks an active profile.
- [x] `VAL-11` GREEN: `git diff --check` passes for root and touched subrepos.
- [x] `VAL-12` RED/GREEN: focused Laravel repair tests prove soft-deleted event/occurrence references do not block approved test-seed aggregate cleanup, but active links still block.
- [x] `VAL-13` GREEN: local `guarappari` repair dry-run after execution reports `invalid: 0` and `residual: 0`.
- [x] `VAL-14` RED/GREEN: focused Laravel repair tests prove repair-approved test-seed aggregate deletion removes account/profile rows from `withTrashed()` and local purge removes previously soft-deleted harness aggregates.
...
| `SCOPE-12` | `Scope` | Capture before/after local runtime evidence for `guarappari`: missing-profile count must return to zero or every residual row must have an explicit non-test remediation note. | runtime | repair dry-run before, execute, and post-repair dry-run | local runtime | passed | Current final evidence: dry-run before refined repair found 22 invalid and 0 residual; execute deleted 22; post-repair dry-run reports scanned 13, invalid 0, residual 0. |
...
| `SCOPE-14` | `Scope` | Refine the repair command so known local test-seed account aggregates are deletable when the only linked-data blockers are soft-deleted event/occurrence references from harness cleanup, while active user, invite, active event, or active occurrence references still block deletion. | code/test | RED `./laravel-app/scripts/delphi/run_laravel_tests_safe.sh tests/Feature/Accounts/AccountMissingProfileRepairCommandTest.php` failed at `test_repair_deletes_known_test_seed_when_only_soft_deleted_event_references_remain`; GREEN same command passed 9 tests, 88 assertions. | backend | passed | `AccountMissingProfileRepairService` now reports total and active event/occurrence references separately; hard blockers remain fail-closed. |
| `SCOPE-15` | `Scope` | Clean all current local `guarappari` missing-profile residuals through the backend-owned command/service path; no invalid tenant-admin Account may remain visible after the repair. | runtime | Dry-run `docker compose exec -T app php artisan accounts:missing-profiles:repair guarappari --chunk=500` returned invalid 22/would-delete 22/residual 0; execute with confirmation deleted_test_seed 22; final dry-run returned scanned 13, invalid 0, residual 0. | local runtime | passed | Cleanup used only the backend command/service path, not direct database mutation. |
| `SCOPE-16` | `Scope` | Hard-delete repair-approved local/test harness account aggregates instead of leaving account/profile soft-delete residue, and provide a backend-owned local purge path for already-soft-deleted harness aggregates. | code/test/runtime | `AccountManagementService::deleteRepairApprovedTestSeedAggregate`; `accounts:test-seeds:purge`; `./laravel-app/scripts/delphi/run_laravel_tests_safe.sh tests/Feature/Accounts/AccountMissingProfileRepairCommandTest.php` | backend/local runtime | passed | Command dry-run found 98 purgeable soft-deleted harness aggregates; execute purged 98; after dry-run scanned 0/residual 0. |
...
| `DOD-06` | `Definition of Done` | `DOD-06` Local `guarappari` missing-profile count is repaired through backend-owned command/service evidence, not direct DB mutation. | runtime | repair command dry-run/execute/after | local runtime | passed | All current invalid local rows removed via service; post-repair dry-run reports invalid 0 and residual 0. |
...
| `VAL-07` | `Validation Steps` | `VAL-07` GREEN: repair command/service execute repairs local `guarappari`, then dry-run reports zero invalid accounts. | runtime | execute command plus after dry-run | local runtime | passed | Refined execute deleted 22 current residuals; post-repair dry-run scanned 13, invalid 0, residual 0. |
...
| `VAL-10` | `Validation Steps` | `VAL-10` GREEN: post-validation backend invariant query proves no live local `guarappari` account lacks an active profile. | runtime/browser | `accounts:missing-profiles:repair guarappari --chunk=500` after refined repair | local runtime/browser | passed | Post-repair dry-run scanned 13, invalid 0, residual 0. |
...
| `VAL-12` | `Validation Steps` | `VAL-12` RED/GREEN: focused Laravel repair tests prove soft-deleted event/occurrence references do not block approved test-seed aggregate cleanup, but active links still block. | test | RED `./laravel-app/scripts/delphi/run_laravel_tests_safe.sh tests/Feature/Accounts/AccountMissingProfileRepairCommandTest.php` failed at would-delete assertion before implementation; GREEN same command passed 9 tests, 88 assertions. | backend | passed | Covers soft-deleted event plus occurrence allowance, active event blocker, and runtime-invite no-restorable cleanup. |
| `VAL-13` | `Validation Steps` | `VAL-13` GREEN: local `guarappari` repair dry-run after execution reports `invalid: 0` and `residual: 0`. | runtime | `docker compose exec -T app php artisan accounts:missing-profiles:repair guarappari --chunk=500` | local runtime | passed | Final output: scanned 13, invalid 0, residual 0, rows empty. |
| `VAL-14` | `Validation Steps` | `VAL-14` RED/GREEN: focused Laravel repair tests prove repair-approved test-seed aggregate deletion removes account/profile rows from `withTrashed()` and local purge removes previously soft-deleted harness aggregates. | test/runtime | `./laravel-app/scripts/delphi/run_laravel_tests_safe.sh tests/Feature/Accounts/AccountMissingProfileRepairCommandTest.php`; `docker compose exec -T app php artisan accounts:test-seeds:purge guarappari --execute --confirm=purge-test-seeds:guarappari --chunk=500` | backend/local runtime | passed | 12 tests, 116 assertions; local purge executed `purged_test_seed: 98`, skipped 0, residual 0; after dry-run scanned 0. |
...
| --- | --- | --- | --- | --- | --- |
| Local Docker Laravel runtime | Needed for repair dry-run/execute and runtime proof. | healthy | 2026-06-01 | `verify_context.sh`, Docker-backed repair dry-run/execute/post-run queries | Use safe local runtime only. |
...
- [x] `D-16` Local residual policy refinement approved on 2026-06-01: known harness/test-seed missing-profile accounts may be account-aggregate deleted when their only linked-data blockers are soft-deleted event/occurrence references; active user, invite, active event, active occurrence, or unknown relation data remains a hard skip.
- [x] `D-17` The exact `runtime-invite-account` harness residue may be account-aggregate deleted when it has no active profile, no restorable profile, and no linked user/invite/active event/active occurrence data, even if its historical `ownership_state` is blank.
- [x] `D-18` Local/test repair-approved harness cleanup must hard-delete account/profile/role-template aggregate rows; this policy is not authorized for production/stage data repair.
...
| `laravel-app / focused repair command tests` | Repair policy branches and dry-run/execute parity. | `./laravel-app/scripts/delphi/run_laravel_tests_safe.sh tests/Feature/Accounts/AccountMissingProfileRepairCommandTest.php` | Local-Implemented | passed | 12 tests, 116 assertions | Covers restore, hard-delete safe test-seed delete, soft-deleted event/occurrence allowance, local purge of already-soft-deleted harness aggregates including unmanaged historical residues, active-event block, linked-data skip, missing type skip, no-restorable skip, runtime-invite cleanup, and confirmation guards. |
...
| `local runtime / post-validation invariant query` | Proves validation did not recreate orphan accounts. | `docker compose exec -T app php artisan accounts:missing-profiles:repair guarappari --chunk=500` | Local-Implemented | passed | after refined repair: scanned 13, invalid 0, residual 0, rows empty | No missing-profile account remains visible in the local Guarapari tenant-admin dataset. |
...
| `D-06` | `Adherent` | Laravel tests, source scan, Playwright shards, post-run repair dry-run | Prevention and current local repair both evidenced; final Guarapari repair dry-run reports invalid 0 and residual 0. |
| `D-16` | `Adherent` | `AccountMissingProfileRepairCommandTest`; `AccountMissingProfileRepairService` linked-data split | Soft-deleted event/occurrence references no longer keep known test-seed accounts visible; active event/occurrence, user, and invite blockers remain fail-closed. |
| `D-17` | `Adherent` | `AccountMissingProfileRepairCommandTest::test_repair_deletes_runtime_invite_account_without_restorable_profile_when_no_links`; Guarapari runtime execute | Exact runtime invite harness residue can be deleted only when no active profile, no restorable profile, and no hard linked data exist. |
| `D-18` | `Adherent` | `AccountMissingProfileRepairCommandTest`; `accounts:test-seeds:purge guarappari` dry-run/execute/after | Repair-approved local/test harness cleanup now force-deletes aggregate rows and the local purge path removed soft-deleted harness residue without authorizing production/stage repair. |
