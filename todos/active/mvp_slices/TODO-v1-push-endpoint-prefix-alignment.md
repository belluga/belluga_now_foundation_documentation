...
## Tasks
- [ ] ⚪ Identify the current push transport base URL assembly path.
- [ ] ⚪ Update the push transport config to use `{baseUrl}/v1` (keeping other adapters intact).
- [ ] ⚪ Verify push endpoints resolve to `/api/v1/push/*`.
- [ ] ⚪ Add temporary logging/interceptor to print resolved push register URL + auth header presence.
- [ ] ⚪ Run Flutter unit/widget tests and targeted integration test(s) or device run to capture the log output.
- [ ] ⚪ Remove temporary logging once the URL is confirmed.
