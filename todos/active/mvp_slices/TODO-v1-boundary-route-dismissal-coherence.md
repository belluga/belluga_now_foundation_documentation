...
- `D-07`: Warm tenant-public map entry may not rely on unresolved `guard -> redirectUntil(...)` alone, because that model does not commit a browser-history entry before the permission boundary on web. For visible user-initiated map entry, Flutter must push `/location/permission` explicitly as a normal result-return route; the permission screen returns `granted|continueWithoutLocation|cancelled` through AutoRoute `pop(result)`, and the caller pushes `/mapa` only for the success/soft-gate outcomes. Route guards remain authoritative for direct URL/deep-link entry.
