...
The required direction is a single group authoring pattern for Account Profile groups, event-level related profiles, and occurrence-owned related profiles. Event and occurrence groups define the final public tabs. The existing `event_parties` relation still has a job: it remains the backend/materialized relation for who is linked to the event or occurrence, including metadata, party type, permission/projection semantics, and compatibility consumers.
...
- **Current delivery stage:** `Web-Validated / Delivery-Gate-Reconciliation`
- **Qualifiers:** `Feature`, `Cross-Stack`, `Tenant-Admin`, `Tenant-Public`, `Events`, `User-Visible`, `Contract-Driven`, `Requires-APROVADO`
- **Next exact step:** review this TODO contract and approve it with `APROVADO`; implementation must not continue until approval.
...
- [ ] Public event detail renders explicit event/occurrence group labels and order as the final tabs.
- [ ] Public event detail falls back to Account Profile type plural labels only when no explicit groups exist for that effective scope.
- [ ] Legacy fallback is scoped independently: event-level fallback and selected-occurrence fallback can each apply without forcing the other scope into fallback.
...
- [ ] Public selected occurrence detail renders event-level groups plus the selected occurrence's own groups only.
...
- [ ] Public selected occurrence detail never leaks profile groups from another occurrence.
...
- [ ] `D-EVG-06` Public selected occurrence detail renders event groups plus selected-occurrence groups only.
...
- [ ] `D-EVG-10` Legacy fallback is evaluated per scope. An event-level scope can use explicit groups while the selected occurrence falls back by type, and the inverse is also valid.
...
| `DP-EVG-03` | When event-level and selected-occurrence groups share the same `id`, should public detail merge them? | Merge same `id`, preserve event order first, append occurrence-only groups after. | Avoids duplicate tabs named the same thing while keeping occurrence data local. |
...
### Public UI Rules
- [ ] If explicit event groups exist, event-level related-profile tabs use those group labels and order.
- [ ] If explicit selected-occurrence groups exist, selected-occurrence profile tabs use those group labels and order.
- [ ] If both event and selected-occurrence groups exist, the final UI renders the effective combined tab set without duplicate cards.
- [ ] If no explicit groups exist for an effective scope, fallback tabs are derived by Account Profile type plural label.
- [ ] Fallback is independently scoped. Event explicit groups plus selected-occurrence fallback must render correctly; event fallback plus selected-occurrence explicit groups must also render correctly.
...
| `BE-EVG-13` | Public selected occurrence merges event groups and selected occurrence groups. | Final projection includes event groups plus selected occurrence groups only. | Public detail feature test. |
| `BE-EVG-14` | Another occurrence has different groups. | Non-selected occurrence groups do not appear. | Public detail feature test. |
...
| `BE-EVG-19` | Selected occurrence has legacy occurrence-owned `event_parties` and no occurrence groups. | Public selected occurrence projection falls back to type-plural tabs for that occurrence scope. | Compatibility feature test. |
| `BE-EVG-20` | Event has explicit groups and selected occurrence has legacy occurrence-owned `event_parties`. | Public projection renders event explicit tabs plus selected-occurrence fallback tabs. | Public detail feature test. |
| `BE-EVG-21` | Event has legacy event-level `event_parties` and selected occurrence has explicit groups. | Public projection renders event fallback tabs plus selected-occurrence explicit tabs. | Public detail feature test. |
...
| `FL-EVG-14` | Public event detail controller applies a selected occurrence to an event with explicit groups. | Rebuilt selected-occurrence event state preserves event-level `profileGroups`; it never falls back by type because the controller dropped groups. | Controller regression test. |
...
| `PUB-EVG-04` | Selected occurrence has own group `Artistas`. | Selected occurrence tab appears with occurrence members. | Public widget/navigation test. |
| `PUB-EVG-05` | Switch selected occurrence. | Previous occurrence groups disappear; new selected occurrence groups appear. | Public navigation test. |
...
| `PUB-EVG-09` | Event has explicit groups and selected occurrence has no explicit groups. | Public event detail shows event custom tabs plus occurrence fallback type-plural tabs. | Public widget/navigation test. |
| `PUB-EVG-10` | Event has no explicit groups and selected occurrence has explicit groups. | Public event detail shows event fallback type-plural tabs plus occurrence custom tabs. | Public widget/navigation test. |
...
| `PUB-EVG-15` | Real browser validates an event with multiple occurrences and different occurrence-owned groups. | Occurrence A renders only its own group/members; occurrence B renders only its own group/members; groups never bleed across selected occurrences. | Browser diagnostic navigation test. |
...
| `MAN-EVG-08` | Tenant Admin | Add an occurrence-owned group and profiles to occurrence A. | Public event shows that group only when occurrence A is selected/effective. |
| `MAN-EVG-09` | Tenant Admin/Public | Add different occurrence-owned group to occurrence B, then switch between occurrences. | Occurrence A and B groups never bleed into each other. |
...
| `MAN-EVG-18` | Tenant Admin/Public | Validate event explicit groups with selected occurrence fallback. | Final public tabs include event custom labels plus occurrence type-plural fallback labels. |
| `MAN-EVG-19` | Tenant Admin/Public | Validate event fallback with selected occurrence explicit groups. | Final public tabs include event type-plural fallback labels plus occurrence custom labels. |
...
| Public event mixed explicit/fallback scopes work | Public compatibility | `shared-android-web` | Laravel + Flutter widget + browser read-only | `no` | Event explicit + occurrence fallback and event fallback + occurrence explicit evidence. |
...
| --- | --- | --- | --- | --- | --- | --- |
| `laravel-app` focused Events feature tests | Backend validation/materialization/projection changes are core to this TODO. | `./laravel-app/scripts/delphi/run_laravel_tests_safe.sh tests/Feature/Events/EventCrudControllerTest.php --filter=profile_groups` | `Local-Validated` | passed | `foundation_documentation/artifacts/v0.2.0-plus8/full-ci-equivalent-20260601/laravel-event-profile-groups-focused-rerun.log` | 9 tests / 67 assertions passed; covers grouped write validation, legacy fallback, management hydration, occurrence locality, and public mixed fallback/explicit projections. |
| `laravel-app` related projection/consumer tests | Compatibility arrays and public DTO consumers must not regress. | Focused Events public projection tests, exact file/filter to be selected during implementation. | `Local-Validated` | passed | `foundation_documentation/artifacts/v0.2.0-plus8/full-ci-equivalent-20260601/laravel-event-profile-groups-focused-rerun.log` | Public detail tests in the focused Events suite covered profile group projection, event fallback, occurrence fallback, and selected-occurrence merge behavior. |
| `flutter-app` tenant-admin event domain/controller tests | New admin state must have one mutable source and deterministic derived payloads. | Focused `fvm flutter test --no-pub` for tenant-admin event form state/encoder/controller files. | `Local-Validated` | passed | `foundation_documentation/artifacts/v0.2.0-plus8/full-ci-equivalent-20260601/flutter-event-profile-groups-focused-rerun.log` | 132 focused Flutter tests passed across encoder, DTO, controller, admin form, and public event detail. |
| `flutter-app` shared group editor tests | Prevents duplicated implementations and validates dropdown/search/filter behavior. | Focused `fvm flutter test --no-pub` for shared profile group editor tests. | `Local-Validated` | passed | `foundation_documentation/artifacts/v0.2.0-plus8/full-ci-equivalent-20260601/flutter-event-profile-groups-focused-rerun.log`; `foundation_documentation/artifacts/v0.2.0-plus8/full-ci-equivalent-20260601/web-navigation-mutation-admin-final-event-profile-groups.log` | Admin form tests cover grouped selector checkbox state, empty state, local search/filter, later-page candidate selection, and occurrence group mutation; browser `admin-final` passed nested Account Profile capability flow. |
| `flutter-app` public event detail tests | Final event UI is the user-visible delivery. | Focused `fvm flutter test --no-pub` for immersive/public event detail grouped tabs. | `Local-Validated` | passed | `foundation_documentation/artifacts/v0.2.0-plus8/full-ci-equivalent-20260601/flutter-event-profile-groups-focused-rerun.log`; `foundation_documentation/artifacts/v0.2.0-plus8/full-ci-equivalent-20260601/web-navigation-readonly-event-profile-groups.log` | Public event detail widget tests passed; readonly browser navigation passed 16/16 on the final web bundle. |
| `flutter-app` analyzer | Domain/UI refactor risk. | `cd flutter-app && fvm flutter analyze --no-pub` | `Local-Validated` | passed-with-known-info | `foundation_documentation/artifacts/v0.2.0-plus8/full-ci-equivalent-20260601/flutter-analyze-event-profile-groups-rerun.log` | Only the pre-existing duplicated `constant_identifier_names` info in `assets/fonts/boora_icons_source/icomoon/boora_icons_icomoon.dart:19` remains. |
| `flutter-app` rule matrix | Architecture and lint guardrails must remain satisfied. | `cd flutter-app && bash ${PACED_GLOBAL_ANALYZER_PLUGIN_DIR:-tool/belluga_analysis_plugin}/bin/validate_rule_matrix.sh` | `Local-Validated` | passed | `foundation_documentation/artifacts/v0.2.0-plus8/full-ci-equivalent-20260601/flutter-rule-matrix-event-profile-groups.log` | Expected 58 lint codes detected; total distinct emitted 59. |
| `flutter-app` web build | Browser/manual validation must use the built version. | `cd flutter-app && CLEAN_OUTPUT=1 BUILD_HEARTBEAT_SECONDS=30 bash scripts/build_web.sh ../web-app dev --clean-output` | `Web-Validated` | passed | `foundation_documentation/artifacts/v0.2.0-plus8/full-ci-equivalent-20260601/flutter-web-build-event-profile-groups-rerun.log` | Final build used the project script and published `../web-app` lane `dev`; readonly browser evidence reported `buildSha=e29764f4`. |
| Web navigation mutation | Admin create/edit flow needs browser evidence. | `NAV_WEB_SHARD=admin-final NAV_WEB_WORKERS=1 PLAYWRIGHT_IGNORE_HTTPS_ERRORS=true bash tools/flutter/run_web_navigation_smoke.sh mutation` or a narrower approved shard/test. | `Web-Validated` | partial-passed | `foundation_documentation/artifacts/v0.2.0-plus8/full-ci-equivalent-20260601/web-navigation-mutation-admin-final-event-profile-groups.log`; `foundation_documentation/artifacts/v0.2.0-plus8/full-ci-equivalent-20260601/web-navigation-mutation-occurrences-event-profile-groups.log` | Existing deterministic mutation shards passed: `admin-final` 9/9 and `occurrences` 2/2. Dedicated EVG browser diagnostic now also opens real admin edit for new grouped and legacy no-group events, but event creation itself is still API-seeded rather than fully authored through the create form. |
| Web navigation admin/public diagnostic | User-requested real navigation for new grouped events, legacy events without groups, inconsistent historical data, and multi-occurrence selected occurrence behavior. | `NAV_RUNTIME_DB_MUTATION_ALLOWED=1 NAV_WEB_TEST_TYPE=diagnostic npx playwright test --config ./playwright.config.js ../web_app_tests/event_profile_groups_runtime.diagnostic.spec.js --grep '@diagnostic' --workers 1 --reporter=line` from `tools/flutter/web_app_smoke_runner`. | `Web-Validated` | passed | Local command output: `1 passed (2.2m)` on 2026-06-02 after web rebuild. | Creates distinct Account Profile types with plural labels that differ from custom tab labels; seeds new/legacy/inconsistent events plus a two-occurrence event with different occurrence-owned groups; validates admin edit groups/counts and public final tabs/cards on `https://guarappari.belluga.space`. |
| Web navigation read-only | Public event final UI needs browser evidence. | `PLAYWRIGHT_IGNORE_HTTPS_ERRORS=true bash tools/flutter/run_web_navigation_smoke.sh readonly` or a narrower approved public event test. | `Web-Validated` | passed | `foundation_documentation/artifacts/v0.2.0-plus8/full-ci-equivalent-20260601/web-navigation-readonly-event-profile-groups.log`; local EVG diagnostic command above. | 16/16 readonly browser tests passed against `https://belluga.space` and `https://guarappari.belluga.space` on lane `dev`; EVG diagnostic additionally proved custom-vs-type-label tabs, legacy fallback tabs, and invalid historical suppression. |
...
- `Real browser EVG diagnostic`: after rebuilding web with `CLEAN_OUTPUT=1 BUILD_HEARTBEAT_SECONDS=30 bash scripts/build_web.sh ../web-app dev --clean-output`, ran `NAV_RUNTIME_DB_MUTATION_ALLOWED=1 NAV_WEB_TEST_TYPE=diagnostic npx playwright test --config ./playwright.config.js ../web_app_tests/event_profile_groups_runtime.diagnostic.spec.js --grep '@diagnostic' --workers 1 --reporter=line`; final result `1 passed (2.2m)` after harness cleanup was expanded to remove diagnostic events, onboarded accounts, and Account Profile types. Covered: new event with custom groups whose labels differ from Account Profile type plurals and include mixed types per group; legacy event with `event_parties` and no groups; historical invalid group-only member suppressed from public UI; two-occurrence event where selected occurrence A renders only `Palco Sexta`/its profiles and selected occurrence B renders only `Palco Sabado`/its profiles; admin edit and public detail navigation on `https://guarappari.belluga.space`.
...
6. Add fail-first Flutter public event UI/navigation tests for explicit tabs, fallback tabs, mixed types, selected occurrence isolation, card navigation, and no infinite loading.
