...
- **Qualifiers:** `Planning-Ready`, `Feature`, `Cross-Stack`, `Tenant-Public`, `User-Visible`, `Depends-On-Reference-Core`
- **Next exact step:** verify the reference-location core blocker status, then request `APROVADO` for this action slice or orchestrate blocker closure first if still incomplete.

## Scope
- [ ] Add a text+icon action below current CTA buttons on the map POI card.
- [ ] Add a hero action on Account Profile detail when the profile/type has effective `is_reference_location_enabled=true`.
- [ ] Save the selected POI/Profile as the user's fixed `ponto de referência` using the existing proximity preference contract.
- [ ] Persist entity-backed reference metadata: `source_kind`, `entity_namespace`, `entity_type`, `entity_id`, display label, and coordinate snapshot.
- [ ] Persist enough source metadata for downstream route prompts to display the Account Profile name and navigate to the Account Profile when the reference point is Account Profile-backed.
- [ ] Treat saved reference state as user-owned preference state, not Account Profile-owned state.
- [ ] Show a distinct current/selected state when the displayed POI/Profile is already the active `ponto de referência`.
- [ ] Update affected user-visible copy from `origem padrão` to `ponto de referência`.
- [ ] Reset the directions route prompt policy setting to `null` whenever a new `ponto de referência` is selected.
- [ ] Respect disabled/ineligible reference resolution when source capability prerequisites fail.
- [ ] Add tests for map card action, Account Profile hero action, persistence, and selected-state rendering.
...
- [ ] `DEP-01` Hard dependency on `foundation_documentation/todos/active/post_release_hardening/TODO-store-release-reference-location-core-and-dependent-capability-guardrails.md` for disabled-reference payload and capability semantics.
- [ ] `DEP-02` Related to `foundation_documentation/todos/active/post_release_hardening/TODO-store-release-proximity-preferences-and-location-origin.md`; this TODO may consume existing implementation but must not redefine identity merge semantics.
- [ ] `DEP-03` Coordinate settings reset semantics with `TODO-v0.2.0+8-event-directions-inline-provider-actions.md`; this TODO owns the reset when a new `ponto de referência` is selected.

## Definition of Done
- [ ] Map POI cards for eligible account-profile POIs show an action to set/use the POI as the `ponto de referência`.
- [ ] Account Profile hero shows the action only for profiles with effective reference-location capability and usable coordinates.
- [ ] Tapping either action saves the entity-backed fixed reference through the proximity preference path.
- [ ] Saved reference includes coordinate snapshot, generic entity provenance, display label, and route/navigation metadata needed by route prompt consumers.
- [ ] The same POI/Profile renders a selected/current state when it is already the active `ponto de referência`.
- [ ] Selecting a new `ponto de referência` resets the directions route prompt policy setting to `null`.
- [ ] Ineligible profiles do not show the action, or show a disabled state only if explicitly approved during planning.
- [ ] Auth/anonymous behavior is deterministic and matches existing proximity preference repository behavior.
- [ ] Tests cover map card, hero, persistence payload, and current-state rendering.

## Validation Steps
- [ ] Laravel test for persisting entity-backed fixed reference via proximity preferences, including provenance fields.
- [ ] Laravel/Flutter test proving Account Profile-backed references expose enough label/route metadata for prompt display and shortcut navigation.
- [ ] Laravel/Flutter tests for disabled/ineligible reference resolution if the blocker is not already closed with sufficient evidence.
- [ ] Flutter repository/domain test for mapping active fixed reference to UI selected state.
- [ ] Flutter map card widget test for action visibility, tap, and selected state.
- [ ] Flutter Account Profile hero widget test for capability-gated action visibility, tap, and selected state.
- [ ] Analyzer/local CI-equivalent suite rows completed before delivery.
...
| --- | --- | --- | --- | --- |
| `operational-coder` | `assurance-tester-quality` | User-owned preference persistence and public action visibility need focused regression validation. | `flutter-app`, `laravel-app`, tests | `planned` |
...
| --- | --- | --- | --- |
| `laravel-app` | Laravel feature/unit tests | project safe Laravel test runner for proximity/account profile capability tests | `yes` |
