...
- [ ] Flutter/Laravel automated: app/native share-code acceptance requires authenticated identity; anonymous share accept is blocked before backend mutation or rejected with `401 auth_required`.
...
| VAL-17 | Validation Steps | Flutter/Laravel automated: app/native share-code acceptance requires authenticated identity; anonymous share accept is blocked before backend mutation or rejected with `401 auth_required`. | automated + browser mutation | Supersedes earlier anonymous-accept test evidence; refresh with authenticated share-code accept test plus anonymous-boundary/rejection tests before promotion reliance. | local Flutter/Laravel tests + Playwright web mutation | blocked | Policy correction on 2026-04-30 changed the canonical contract. Playwright web boundary evidence remains relevant for anonymous web; app/native authenticated accept evidence must be refreshed. |
