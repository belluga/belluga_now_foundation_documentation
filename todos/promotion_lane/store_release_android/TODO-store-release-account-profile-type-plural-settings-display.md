...
- **Qualifiers:** `Store-Release`, `Flutter`, `Laravel`, `Tenant-Admin`, `Account-Profile-Catalog`, `Settings`, `User-Flow-Impact`, `T6-Orchestration`, `Focused-Flutter-Green`, `Laravel-Green`, `Playwright-Green`
- **Next exact step:** preserve the plural-label closure on the consolidated branch and keep the admin/browser mutation proof attached to the T6 evidence packet.
