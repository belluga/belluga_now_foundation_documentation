...
Manual validation on 2026-06-03 found that the new reference-point confirmation modal is functionally correct but still needs release-polish and an active-state reversal path. These findings are tracked as `PTODO-001`, `PTODO-002`, `PTODO-003`, and `PTODO-005` in `foundation_documentation/artifacts/v0.2.0-plus8/pre-todo-manual-validation-findings-v0.2.0-plus8.md`.
...
- **Current delivery stage:** `Local-Validated / Addendum Evidence Ready`
...
- [x] `PTODO-001` Reference-point modal primary action is full-width and follows the app's standard primary-button visual language.
- [x] `PTODO-002` Reference-point modal cancel action appears below the primary action and is centered.
- [x] `PTODO-003` Reference-point modal copy emphasizes `distâncias` and `calculadas`.
- [x] `PTODO-005` Active `Ponto de referência` state exposes an explicit clear/cancel-reference action.
...
- [x] Active `Ponto de referência` state can be cleared by the user without selecting a different reference point.
- [x] Reference-point modal layout matches the current app button hierarchy: primary full-width action first, secondary cancel below and centered.
...
- [x] Flutter widget/regression tests cover the reference-point confirmation modal layout/copy and clear-reference action.
- [x] Repository/controller test proves clearing the active reference point resets fixed-reference state without corrupting unrelated proximity settings.
...
| Clear active reference point | Public preference mutation | `shared-android-web` | repository/controller + widget | `yes` | clear-reference action test and preference state readback |
| Reference-point modal release polish | Public visible flow | `shared-android-web` | widget | `no` | modal layout/copy widget test |
...
| Proximity preference fixed reference clear action | Flutter map card + Account Profile hero | clear current point-of-reference state | proximity preference repository/controller | Flutter repository/controller/widget tests |
...
| `flutter-app` PTODO reference-point addendum focused tests | `PTODO-001..003` modal layout/copy and `PTODO-005` clear-reference path changed in Account Profile and map POI consumers. | `cd flutter-app && fvm flutter test --no-pub test/infrastructure/repositories/proximity_preferences_repository_test.dart test/presentation/tenant_public/partners/account_profile_detail_screen_test.dart test/presentation/tenant/map/screens/map_screen/controllers/map_screen_controller_test.dart` | `Local-Validated` | passed | command output, 2026-06-03 | Passed 161 tests in the combined affected set; includes repository clear, Account Profile clear action, map deck clear action, and modal copy/layout keys. |
...
- `REF-PTODO-001..003`: reference-point confirmation modal needs release-polish: full-width standard primary action, cancel below/centered, and bold emphasis on `distâncias` and `calculadas`.
- `REF-PTODO-005`: active `Ponto de referência` state needs an explicit clear/cancel action.
- Execution remains inside the approved reference-point objective because it refines the visible confirmation/current-state UX and the same user-owned proximity preference state. If implementation requires a new backend contract beyond clearing the existing fixed-reference value, stop and request renewed approval.
...
| SCOPE-12 | Scope / 2026-06-03 addendum | `PTODO-001..003` reference-point confirmation modal has full-width primary action, centered secondary cancel below it, and emphasized `distâncias`/`calculadas`. | Flutter widget tests plus code review | `account_profile_detail_screen_test.dart`; `map_screen_controller_test.dart`; focused command in Local CI-Equivalent Suite Matrix | Flutter Account Profile and public map widget tests | passed | Dialog content now uses keyed rich copy and full-width primary button in both Account Profile and map POI flows. |
| SCOPE-13 | Scope / 2026-06-03 addendum | `PTODO-005` active reference state exposes a clear/cancel path and restores live-location proximity behavior. | Flutter widget and repository tests | `proximity_preferences_repository_test.dart`; `account_profile_detail_screen_test.dart`; `map_screen_controller_test.dart`; focused command in Local CI-Equivalent Suite Matrix | Flutter repository plus public Account Profile/map widget tests | passed | Active state keeps the `Ponto de referência` indicator and adds explicit `Cancelar ponto de referência`; repository `clearFixedReference` writes live device location. |
...
| DOD-05A | Definition of Done / 2026-06-03 addendum | Active `Ponto de referência` state can be cleared by the user without selecting a different reference point. | widget/repository tests | `proximity_preferences_repository_test.dart`; `account_profile_detail_screen_test.dart`; `map_screen_controller_test.dart` | Flutter repository and public widget tests | passed | Clear action is confirmed in Account Profile and map deck, then persisted through `clearFixedReference`. |
| DOD-05B | Definition of Done / 2026-06-03 addendum | Reference-point modal layout matches app hierarchy: primary full-width action first, secondary cancel below and centered. | widget tests and code review | `account_profile_detail_screen_test.dart`; `map_screen_controller_test.dart` | Flutter public widget tests | passed | Both modals place the primary action in a full-width `SizedBox` above the centered cancel action. |
...
| VAL-08 | Validation Steps / 2026-06-03 addendum | Flutter widget/regression tests cover reference-point confirmation modal layout/copy and clear-reference action. | focused Flutter test | `cd flutter-app && fvm flutter test --no-pub test/infrastructure/repositories/proximity_preferences_repository_test.dart test/presentation/tenant_public/partners/account_profile_detail_screen_test.dart test/presentation/tenant/map/screens/map_screen/controllers/map_screen_controller_test.dart` | Flutter repository and widget tests | passed | Focused addendum rerun passed on 2026-06-03. |
