...
- **Qualifiers:** `Feature`, `Cross-Stack`, `Tenant-Admin`, `Tenant-Public`, `Events`, `User-Visible`, `Promotion-Lane-Pending`
- **Next exact step:** carry this parent capability from `promotion_lane/v0.2.0+8/` through the remaining package-wide promotion follow-through; the event/occurrence expansion authority delegated to `TODO-v0.2.0+8-event-profile-groups-canonical-consistency.md` is now locally validated.

## Active Work State
- **Work state:** `review`
- **Why this state now:** the Account Profile nested-group capability is implemented, and the previously open event/occurrence scope is now owned and locally validated through the child EVG TODO instead of remaining an unfinished parent TODO burden.
- **Exit condition:** package-wide promotion follow-through leaves both the parent capability and the EVG child without reopened findings.
...
- [x] Event and Event Occurrence related Account Profiles can be organized into named profile groups using the same one-level group semantics as Account Profile nested groups.
- [x] Tenant-admin event authoring requires each selected event-level or occurrence-owned related Account Profile to belong to a profile group.
- [x] Public event detail renders grouped related-profile tabs by group label instead of deriving tab titles from Account Profile type plural labels.
- [x] Existing programming semantics are preserved: programming items may reference only Account Profiles already linked to the same occurrence-owned related-profile set, and group membership does not constrain programming selection.
- [x] Existing `linked_account_profiles[]`, occurrence `own_linked_account_profiles[]`, and programming linked-profile projections remain available for hero/image fallback, direct navigation, maps, programming, and compatibility consumers.
...
- [x] Admin can create at least two event-level or occurrence-owned related-profile groups with mixed Account Profile types in one group.
- [x] Public event detail shows those groups as tabs using the saved group labels and group order.
- [x] Public event detail no longer creates separate related-profile tabs from Account Profile type plural labels when explicit groups are present.
- [x] Programação still allows only occurrence-owned related profiles and remains valid when the selected profiles belong to any occurrence group.
...
- [x] Laravel feature tests for event/occurrence profile-group write validation, readback, and public projection.
- [x] Flutter tenant-admin event form tests for grouped related-profile authoring at event and occurrence levels.
- [x] Flutter public event detail DTO/domain/widget tests for grouped tabs and fallback behavior.
- [x] Web navigation mutation/read-only tests that create or edit grouped event related profiles and validate the public event detail tabs.
...
- **Disposition reason:** the package-wide review loop now explicitly carries this parent capability in `promotion_lane/v0.2.0+8/`; the Account Profile nested-group capability and the EVG child are locally validated, and the current package sweep did not reopen a new parent-scope defect beyond the consolidated CI-equivalent gate still pending for the full bundle.
...
- **Next path/status action:** keep this TODO in `foundation_documentation/todos/promotion_lane/v0.2.0+8/`, carry the parent/child capability through the consolidated v0.2.0+8 CI-equivalent gate, and reopen it only if that gate surfaces a real nested/EVG regression.
