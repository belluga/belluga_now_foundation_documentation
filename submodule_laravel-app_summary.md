...
* **Invite share materialization identity-first:** `POST /api/v1/invites/share/{code}/materialize` rejects anonymous identities with deterministic `401 auth_required`, creates or reuses the canonical invite edge for the authenticated user, and leaves accept/decline to the standard `/invites/{invite_id}/accept|decline` flow.
