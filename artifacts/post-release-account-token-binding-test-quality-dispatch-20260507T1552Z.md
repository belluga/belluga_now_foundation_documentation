...
## Goal
Post-correction test-quality audit for RR-AUTH-03 after Poincare. Validate tests cover persisted negative ceiling, wildcard issuer fail-close variants, explicit/current inaccessible account ids, deterministic no-current-account login, push data/actions ability rejection, stale account context adequacy, and membership-removal revocation. Return JSON only.
