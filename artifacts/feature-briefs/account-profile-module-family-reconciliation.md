...
- **Desired outcome:** define a coherent future module family centered on the canonical model (`Account`, `Account Profile`, `Profile Type`, `Static Asset`, `Event`), separate internal module truth from public/product aliases, and sequence the normalization into bounded slices instead of one unbounded cleanup pass.
...
| --- | --- | --- | --- | --- |
| `AMB-01` | Whether `partner_catalog_and_offer_module` is still one coherent future module or really two fronts (`account-profile/static-asset catalog/public surfaces` vs a separate `offer/transaction bridge` concern). | A rename without a boundary decision would just preserve drift under a new label. | Current doc mixes account profiles, offers, map projections, and public detail behavior in one module. | `resolve now` |
| `AMB-02` | Whether `partner_admin_module` should survive as its own module or fold into `account_workspace` and/or `tenant_admin`. | This changes module map, dependencies, and where future operator-facing docs belong. | Current doc is still a placeholder and already frames itself as future Account Profile Admin/Workspace. | `resolve now` |
| `AMB-03` | Whether `partner_analytics_module` is a real future module or just an analytics concern subordinate to workspace/admin flows. | Placeholder analytics docs can become fake authority if their boundary is not explicit. | Current doc is placeholder-only and depends on workspace timing. | `resolve now` |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ST-01` | Decide the canonical future topology and names for the current `partner_*` module family. | `partner_catalog_and_offer_module.md` | `partner_admin_module.md`, `partner_analytics_module.md`, `tenant_admin_module.md`, `project_constitution.md` | The project has an explicit future module-family decision: what remains separate, what folds into workspace/admin, and which names are canonical going forward. | Brief/TODO decision table is internally coherent and maps each current module to one future handling (`preserve/rename/merge/split/retire`). | `create-now` | Requires domain-entity/top-level authority baseline, now satisfied. | Recommended current slice. |
