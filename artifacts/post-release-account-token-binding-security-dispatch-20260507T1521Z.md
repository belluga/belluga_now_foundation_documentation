...
## Goal
Security adversarial review for RR-AUTH-03 after the token-ceiling/issuer fix. Assess broken function authorization, account/tenant breakout, unbound or wrong-account bearer token use, wildcard ceiling abuse, strict account ID comparisons, role-downgrade next-request denial, push data/actions route binding, and non-account route false-denial risk. Return JSON only.
