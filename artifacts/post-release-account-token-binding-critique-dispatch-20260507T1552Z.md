...
## Goal
Post-correction critique for RR-AUTH-03 after Poincare. Verify issuer rejects inaccessible explicit/current account_id, login without current account is deterministic, push data/actions ability contract is enforced, persisted negative ceiling and membership-removal regressions are meaningful, and residual verification debt is correctly classified. Return JSON only.
