...
| MNT-015 | `flutter-app/lib/presentation/tenant_admin/events/screens/tenant_admin_event_form_screen.dart` | Canceled | Venue picking awaits controller-backed candidate hydration before opening an ephemeral picker sheet. The `context.mounted` guard only prevents launching the picker from a disposed form; selection handling stays local and no post-await navigation decision is moved into the widget. | 2026-06-11 | Delphi |
| MNT-016 | `flutter-app/lib/presentation/tenant_admin/events/widgets/tenant_admin_event_occurrence_editor_sheet.dart` | Canceled | Programming-location picking awaits candidate hydration, then updates sheet-local draft state. The `mounted` guard only prevents late modal/UI updates after sheet dismissal; no cross-route navigation or controller ownership drift is introduced. | 2026-06-11 | Delphi |
