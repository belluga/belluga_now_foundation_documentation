...
- Focused RR-AUTH-04 resolver/reset/risk suite:
  - `./scripts/delphi/run_laravel_tests_safe.sh tests/Unit/Application/Auth/TenantPublicAuthMethodResolverTest.php tests/Unit/Application/Auth/PasswordResetTokenServiceTest.php tests/Unit/Application/Profiles/LandlordProfileServiceTest.php tests/Api/v1/Admin/ApiV1AdminProfileTest.php tests/Feature/Tenants/PasswordRegistrationControllerTest.php tests/Feature/Security/ApiSecurityHardeningMiddlewareTest.php`
  - result: `53 passed`, `278 assertions`, duration `28.29s`
...
- Full Laravel CI-equivalent suite:
  ...
  - result: `1406 passed`, `6681 assertions`, duration `851.27s`

## Current Proof Snapshot

| Invariant | Current Baseline Status | Evidence |
| --- | --- | --- |
| Empty/invalid tenant auth subset cannot silently re-enable password auth | `proved_current_baseline` | `TenantPublicAuthMethodResolver` collapses empty/invalid tenant subsets to `phone_otp`, and resolver/unit/feature tests cover both empty-subset and invalid-subset fail-closed behavior. |
| Password reset tokens are hashed, explicit-expiry, and single-use | `proved_current_baseline` | `PasswordResetTokenService`, tenant/landlord profile services, admin profile coverage, and `PasswordRegistrationControllerTest` prove hashed storage, expiry enforcement, and one-time consumption. |
| Public abuse-sensitive auth endpoints have explicit risk-matrix domains | `proved_current_baseline` | `config/api_security.php`, `scripts/architecture_guardrails.php`, and `ApiSecurityHardeningMiddlewareTest` now cover the in-scope public-auth routes. |
...
3. The final merged Laravel CI-equivalent suite is now green on the current integrated auth/identity tranche baseline: `1406 passed`, `6681 assertions`, `851.27s`.
...
Resolve the required RR-AUTH-04 no-context audit lanes, triple audit, and Claude fourth-auditor comparison with explicit approval for reviewer dispatch, or obtain an explicit waiver from the approval authority for any lane that will remain unrun.
