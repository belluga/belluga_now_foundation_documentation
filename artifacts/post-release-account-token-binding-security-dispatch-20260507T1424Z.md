...
## Goal
Security adversarial review for RR-AUTH-03 account-scoped token/ability binding. Assess broken function authorization, account/tenant breakout, unbound or wrong-account bearer token use, push data/actions route binding, live role revocation through account-scoped tokenCan revalidation, non-account route false-denial risk, and residual token metadata exposure. Return JSON only.
