...
## Goal
Post-fix test-quality audit for RR-AUTH-03. Validate that tests would catch missing account_id, mismatched account_id, stale account-context false denial, live role permission revalidation, and push data/actions middleware bypass regressions; flag weak status-only assertions only when they hide required behavior.
