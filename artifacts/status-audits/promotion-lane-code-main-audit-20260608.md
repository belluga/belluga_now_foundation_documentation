...
| `TODO-store-release-web-to-app-conversion-gate.md` | `main-absorbed (strong); split archival catch-up executed` | `origin/main` now hardcodes `AppPromotionExperience.appDownload`, renders only active publication store targets, keeps `/open-app` redirect-path preservation, and carries the promoted web-to-app policy/module rules. On `2026-06-08`, the delivered slice was moved to `todos/completed/` and the residual Android runtime/startup proof was later absorbed into `TODO-v0.2.0+8-tenant-public-bootstrap-and-startup-boundary-hardening.md`. | dirty | moved delivered slice to `todos/completed/`; residual proof later absorbed into the active `v0.2.0+8` owner |
