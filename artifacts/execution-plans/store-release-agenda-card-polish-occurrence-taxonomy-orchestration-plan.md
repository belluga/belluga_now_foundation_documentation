...
| `T5-DOD-10` | TODO :: Definition of Done :: Focused Flutter tests pass for Boora icon asset-contract coverage, catalog coverage, icon picker list coverage, and legacy storage-key aliases. | Flutter visual/catalog worker | Boora icon asset-contract coverage, catalog coverage, icon picker list coverage, legacy storage-key aliases | `BooraIcons`, `MapMarkerIconToken`, tenant-admin picker catalog, font-family registration, IcoMoon export + hashed runtime font contract, picker chip glyph rendering contract, and Android glyph rendering validation updated from `assets/fonts/boora_icons_source/icomoon/boora.icomoon.json`. Runtime web asset is `assets/fonts/BooraIcons-133da979.ttf` to avoid immutable CDN cache reuse. | Focused Flutter asset/catalog/widget tests passed for uploaded font family/codepoints, all 57 icons, IcoMoon export/runtime font parity, visible picker chip `IconData` embedding with no checkmark replacement, legacy aliases, and runtime filename hash guard; `integration_test/feature_boora_icon_font_rendering_test.dart` passed via `flutter drive` on `192.168.15.9:5555` with flavor `guarappari`. | Local Playwright font probe artifact `foundation_documentation/artifacts/tmp/web-navigation/t5-boora-font-runtime-playwright-20260501.md` loaded `BooraIcons-133da979.ttf` from nginx, verified SHA-256 `133da979...`, `FontManifest` families `BooraIcons` and legacy `Boora`, and canvas alpha pixels for private-use glyphs. | `passed` |
