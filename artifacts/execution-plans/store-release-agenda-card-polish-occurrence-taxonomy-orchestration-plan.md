...
| --- | --- | --- | --- |
| `T5-AGENDA-POLISH` | `foundation_documentation/todos/active/store_release_android/TODO-store-release-agenda-card-polish-and-occurrence-taxonomy-overrides.md` | New Store Release slice for agenda/event card polish, programming item end time, and occurrence taxonomy override/filtering. | Approved on 2026-05-01; can start now. |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `T5-DOD-01` | TODO :: Definition of Done :: All acceptance criteria have concrete evidence in the Completion Evidence Matrix. | Reconciliation/evidence worker | Completion Evidence Matrix rows | Filled in TODO after implementation. | TODO guard pending final run. | n/a | `passed` |
...
| `T5-DOD-10` | TODO :: Definition of Done :: Focused Flutter tests pass for Boora icon catalog coverage, icon picker list coverage, and legacy storage-key aliases. | Flutter visual/catalog worker | Boora icon catalog coverage, icon picker list coverage, legacy storage-key aliases | `BooraIcons`, `MapMarkerIconToken`, and tenant-admin picker catalog updated. | Focused Flutter catalog/widget tests passed for all 55 icons and legacy aliases. | Widget evidence sufficient. | `passed` |
...
| `T5-FLUTTER-ICON-01` | TODO :: current Boora icon font is replaced and tenant-admin picker lists every new font icon. | Flutter visual/catalog worker | `assets/fonts/BooraIcons.ttf`, `BooraIcons`, `MapMarkerIconToken`, icon picker | Font asset and catalog updated; staging folder removed. | Flutter catalog/widget tests passed for 55 icons and aliases. | Widget evidence sufficient. | `passed` |
| `T5-FLUTTER-MAP-WEB-01` | TODO :: Map route family skips tenant-public web max-width frame while non-map routes stay constrained. | Flutter visual/catalog worker | `TenantPublicWebDesktopFrame`, `CityMapRoute`, `PoiDetailsRoute`, web/browser route wrapper | Explicit full-width route exemption implemented. | Flutter widget tests passed for Map/Poi and non-map routes; Playwright runtime proved `/mapa` full-width while Home stayed framed. | `tools/flutter/web_app_tests/map_full_width.spec.js` passed on `https://guarappari.belluga.space` after web build SHA `b6986302` was served. | `passed` |
...
- `WS-T5-E QA Runtime` creates runtime/browser/device checks after implementation branches reconcile and web build is fresh.
...
| `WS-T5-E QA Runtime And Guardrails` | Playwright/browser, optional ADB screenshot/interaction, test-quality audit, performance/query review | reconciled implementation from B/C/D | validation checkpoint | Playwright list/run after web build, ADB if needed, audit artifacts |
...
- **Browser-facing web provenance:** `https://guarappari.belluga.space/index.html` and `https://belluga.space/index.html` both exposed `window.__WEB_BUILD_SHA__ = "b6986302"`.
...
- **Scoped Playwright result:** passed; Home stayed framed at `430px`, `/mapa` occupied the full `1200px` viewport, runtime provenance matched `buildSha=b6986302` and `landlordHost=belluga.space`; evidence artifact `foundation_documentation/artifacts/tmp/web-navigation/t5-map-web-full-width-playwright-20260501.md`.
