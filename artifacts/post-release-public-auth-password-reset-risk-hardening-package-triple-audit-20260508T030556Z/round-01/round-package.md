...
- **Orchestration status:** `corrected-baseline-validated-audit-floor-reconciliation-active`
...
- Focused RR-AUTH-04 resolver/reset/risk suite:
  ...
  - result: `136 passed`, `799 assertions`, duration `96.82s`
- Impacted auth consumer suite:
  ...
  - result: `81 passed`, `441 assertions`, duration `27.57s`
...
- Full Laravel CI-equivalent suite:
  ...
  - result: `1420 passed`, `6835 assertions`, duration `862.15s`
...
| Password reset tokens are hashed, explicit-expiry, single-use, and scope-isolated | `proved_current_baseline` | `PasswordResetTokenService` now uses tenant-scoped slot/cooldown/work-factor identities, slot-scoped atomic upsert/delete semantics with hashed lookup keys, generic pre-auth telemetry normalization, and shared profile-service reuse that revokes persisted Sanctum authenticators on successful reset, while tenant and landlord coverage prove stale-token rejection, one-time consumption, and successful login after the replacement password is set. |
...
3. The final merged Laravel CI-equivalent suite is now green on the current integrated auth/identity tranche baseline: `1420 passed`, `6835 assertions`, `862.15s`.
