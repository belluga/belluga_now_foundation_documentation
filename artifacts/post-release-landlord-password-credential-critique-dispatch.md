...
## Goal
Bounded critique of RR-AUTH-01. Focus on correctness, structural soundness, adherence to the canonical rule that credentials(provider=password) is the exclusive landlord password authority, drift-prevention completeness across all landlord password/email mutation paths, and whether the current package evidence is sufficient for closure without reopening unrelated auth scope.
