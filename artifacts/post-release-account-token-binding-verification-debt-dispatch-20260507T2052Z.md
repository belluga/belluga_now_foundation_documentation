...
## Goal
Post-normalization verification debt audit for RR-AUTH-03. Evaluate whether the normalized packet now carries one authoritative current-baseline status for VDA-002 and VDA-005, an explicit inline debt scan result, and the active audit-floor anchor. Return JSON only.
