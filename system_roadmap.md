...
**Attribution requirement:** External share links carry a single `code` as a GET param that resolves `{ tenant_id, event_id, inviter_principal }`. Web/app flows must preserve this `code` through install/login/signup and bind attribution through authenticated `POST /api/v1/invites/share/{code}/accept`. Any additional post-install binding contract beyond this endpoint requires an explicit contract update.
