...
| `/api/v1/invites` | MOD-201 | Invite feed and referral graph. | Defined | Enforces 1 invite per person/canonical target (`event_id + occurrence_id | null`); **user invites** limited to contacts/installed users; **account_profile invites** can target favorites/followers (admin-assigned in MVP). No route registered in `routes/api`. |
...
| `/api/v1/invites/share` | MOD-201 | External share codes for event invites (new user install/signup attribution). | Planned | Anyone who can invite can generate; resolves to `inviter_principal` (user or account_profile) + canonical target ref (`event_id + occurrence_id | null`); requires `account_profile_id` when inviter is account_profile. Acceptance is bound through `/api/v1/invites/share/{code}/accept`; `/consume` remains deferred. |
