...
| `/admin/api/v1/media/map-filter-image` | Tenant Admin | Upload tenant-scoped image used by public Map discovery-filter `image_uri` and marker image payloads. | Tested & Ready | Complements the canonical discovery-filter editor; accepts authenticated multipart (`key`, `image`) and returns canonical `image_uri`. |
