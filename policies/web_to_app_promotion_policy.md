...
| Module | Impact |
|--------|--------|
| Invite & Social Loop | Web unauth shows context + deep links only; acceptance remains app-only in V1. Web-auth account profile workspace can view invite metrics and manage invite campaigns (post‑MVP), but tenant acceptance remains app-only. |
