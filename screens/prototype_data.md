...
- **Payload de convite:**
  - `id`, `event_id`, `invitee_id`, `inviter_principal` (`{ kind: user|partner, id }`), `status` (`pending`/`accepted`/`declined`; `expired` derivado), `sent_at`, `expires_at` (fim do evento), `inviter_name`, `host_name`, `message`, `image_uri`, `priority_rank`.
