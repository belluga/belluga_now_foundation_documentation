...
| `HOM-06` | Approved | Home V1 auto-classifies the effective origin against the tenant default origin using the tenant-configured maximum Home radius as the boundary, persisting the chosen Home-only mode locally/device-side (`live` vs `fixed reference`) and surfacing that mode explicitly below the logo. | Keeps Home usable for out-of-city users without leaking the rule into Discovery/Map/Event Search before an explicit broader decision. | `foundation_documentation/todos/completed/TODO-v1-home-location-origin-reference-mode.md` |
