...
* **Tenant-Public Desktop Web Mobile-Frame Contract:** On wide desktop web viewports, the shared tenant-public mobile-first route family (`/`, `/descobrir`, `/parceiro/:slug`, `/static/:assetRef`, `/agenda/evento/:slug`, `/invite?code=...`, `/convites?code=...`, `/baixe-o-app`, `/location/permission`, and the web identity boundary that resolves through app-promotion) must render inside one centered mobile-width frame instead of stretching full width. The current shared max width is `430` logical pixels. `/mapa` and `/mapa/poi` are the approved exception because both render the full map workspace and must occupy available browser width. This is a presentation-shell rule only: controller ownership, route guards, route semantics, and web-to-app promotion behavior remain unchanged. Landlord, tenant-admin, and workspace shells are explicitly excluded from this framing rule.
...
* **Event Hero Fallback Contract:** Tenant-public event hero/image resolution is deterministic and contract-driven: use `event.thumb` first, then `linked_account_profiles` in canonical order (`cover_url -> avatar_url` per profile), then `venue` media. Laravel owns this resolution through the canonical event hero resolver for backend-composed payloads/metadata; Flutter must consume the resolved payload contract and must not fall back to legacy `artists` when deriving event hero imagery or palette inputs.
...
* **Boora Map Marker Icon Contract:** The app-owned Boora icon font is the canonical custom icon font for map marker/type visual selection. Flutter exposes it through the `BooraIcons` class while the runtime font family must match the uploaded IcoMoon font asset family. Tenant-admin icon pickers must list every icon declared by the current font catalog, and automated coverage must validate the IcoMoon project JSON, generated Dart reference, source TTF/OTF, hashed runtime TTF filename, TTF family, TTF codepoint ranges, pubspec registration, visible picker chip glyphs, and legacy storage-key aliases so catalog tests cannot pass while glyphs fail at runtime. The runtime font filename must include the source font hash prefix so immutable web/CDN caches cannot serve stale font bytes after a font replacement. If a later upload is SVG-only rather than a full IcoMoon export, that deviation must be explicitly documented before generating runtime assets from it.
...
* **Favorites Strip Preview Contract:** Tenant-home favorites remains snapshot-backed. For `account_profile` targets, the preview block must carry `avatar_url`, `cover_url`, and `profile_type`; Flutter resolves `type` label/visual from the app bootstrap registry and applies the compact precedence `avatar > cover > type visuals`. Event presence emphasis is also snapshot-driven: no halo with no event, subtle halo for upcoming event, stronger halo for live-now event. Favorites navigation now follows the canonical public-detail contract: account-profile favorites navigate only when `can_open_public_detail=true` and `public_detail_path` is present; otherwise the account-profile target is unavailable by design and must not fall back to raw slug synthesis.
...
| Stackable tenant-public detail screen (`/parceiro/:slug`, `/agenda/evento/:slug`, `/static/:assetRef`) | Resolve its same-feature detail controller through `RouteInstanceScope.read<T>(context)`/`RouteInstanceScope.get<T>(context)` inside a route wrapped by `RouteScopedResolverRoute`; route-owned dialogs/sheets must use route-scoped overlay helpers. | Direct global `GetIt.I.get<...DetailController>()` from the screen/detail subtree; passing detail controllers through route/screen constructors; using global GetIt scopes that can bleed across sibling route instances. |
...
| Stackable detail controller identity | `RouteScopedResolverRoute` owns one `RouteInstanceScope` per route instance; descendants, dialogs, bottom sheets, and related route-owned flows resolve the originating route store. | Sibling/parent/child detail routes sharing one mutable detail controller instance; overlays resolving fresh/global detail controllers. | `route_scoped_detail_controller_getit_forbidden`, route-scope widget tests |
...
| `/auth/otp/verify` | POST | Verifies OTP, upgrades to registered phone identity, merges supplied anonymous identities, and returns the authenticated token/profile payload. The same endpoint may also accept the dedicated review-phone credential configured in backend-private admin settings; no separate client route or alternate auth UI is introduced for review access. | Anonymous/app identity | `PhoneOtpVerifyRequest` | `PhoneOtpVerifyResponse` |
...
| `/invites/sent-statuses` | GET | Retrieves occurrence-scoped sent invite button state for the authenticated inviter. Not part of post-auth global hydration. | Tenant | `SentInviteStatusesRequest` | `SentInviteStatusesResponse` |
...
| Web/browser compatibility suite | Invite landing, preview, auth redirect preservation, fallback behavior, `.well-known`/deeplink artifacts | Yes for browser boundary | `readonly` can run on `local|dev|stage|main`; `mutation` can run on `local|dev|stage` and is forbidden on `main`; `diagnostic` is local-only and requires explicit `NAV_DEPLOY_LANE=local` plus explicit `NAV_RUNTIME_DB_MUTATION_ALLOWED=1` because it uses controlled local Docker-backed mutation helpers | Executed through `tools/flutter/run_web_navigation_smoke.sh` and must target the browser-facing domain for the current lane. `diagnostic` is a local pre-promotion gate for runtime/browser paths that need deterministic local mutation and is not a substitute for remote readonly/mutation lane evidence. |
...
| `FCX-15` | Approved | Account-user bearer tokens are bound to the selected account context: Laravel stamps `account_id`, account-scoped routes reject missing/mismatched current-account context, and Sanctum abilities are only a ceiling over live current-account role permissions. | Prevents multi-account permission bleed while keeping Flutter/account-workspace consumers aligned to the backend-selected account context instead of copied ability strings. | Section `2.1`; `TODO-post-release-account-scoped-token-ability-binding.md` |
| `FCX-16` | Approved | Stackable tenant-public detail routes own per-route detail controller identity through `RouteScopedResolverRoute` + `RouteInstanceScope`; descendants and route-owned overlays resolve the originating route store. | Prevents parent/child/sibling detail screens from sharing or recreating the wrong controller instance while preserving AutoRoute resolver hydration and ModuleScope registration. | Sections `2.1.1`, `2.1.2`; `TODO-v0.2.0+8-route-scoped-detail-controller-resolution.md` |
...
| `TODO-store-release-funnel-metrics-validation.md` | Store-release funnel metrics validation | Completed | `2.6` | Freezes client-side telemetry attribution rules for web promotion and Android deferred-capture events; published sink/runtime verification continues in the dedicated hardening TODO. |
| `TODO-store-release-phone-otp-auth-and-contact-match.md` | Phone-OTP auth upgrade and identity handoff | Completed | `2.1`, `7` | The post-auth hydration handoff after registered identity emission is now archival-complete. |
| `TODO-store-release-home-favorites-refresh-regression.md` | Home/account-profile favorite refresh regression | Completed | `2.1`, `7` | User-linked favorite state now participates in post-auth hydration and clears stale empty remote results under the archived contract. |
...
| `TODO-post-release-account-scoped-token-ability-binding.md` | Account-scoped token/ability binding | Implementation checkpoint reconciled | `2.1`, `7` | Promotes the hybrid account binding model: account-user tokens carry `account_id`, account routes fail closed on current-account mismatch, and live role permissions revalidate the Sanctum ability ceiling. |
