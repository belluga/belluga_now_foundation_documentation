...
* **Boora Map Marker Icon Contract:** The app-owned `BooraIcons` font family is the canonical custom icon font for map marker/type visual selection. Tenant-admin icon pickers must list every icon declared by the current font catalog, while existing persisted storage keys resolve through catalog aliases instead of being renamed or dropped during font replacement.
