...
* **Tenant-Public Safe Back Contract:** tenant-public discovery, public-detail, and public-map surfaces must use one centralized safe-back policy: consume route-local UI state first when the screen explicitly owns it, then remove the previous route if stack history exists, and only then execute the route-specific fallback when the route is root-opened. The approved no-history fallback matrix is `/descobrir -> /`, `/agenda -> /profile`, `/agenda/evento/:slug -> /agenda`, `/parceiro/:slug -> /descobrir`, `/static/:assetRef -> /descobrir`, `/mapa -> /`, `/mapa/poi -> /mapa`. System back, visible back buttons, and shared immersive-detail back actions must all delegate to the same contract.
* **Canonical Route Back Governance Contract:** Flutter route back ownership is structural and explicit. Root-openable tenant-public surfaces must build back through `buildTenantPublicSafeBackPolicy(...)` with an explicit fallback and non-null `reentrancyKey`; tenant-admin internal screens/forms/details/settings must build or close through `buildTenantAdminCurrentRouteBackPolicy(...)` / `performTenantAdminCurrentRouteBack(...)`; shared back-owning widgets may accept injected handlers/policies but must not hide raw route policy defaults. Route-policy `PopScope` ownership belongs to `RouteBackScope`, not to ad-hoc screen code. Result-return or overlay-only flows (for example `/location/permission`) remain explicitly exempt until they are migrated into a typed route-policy form.
...
| `FCX-06` | Approved | Tenant-public direct-entry routes must use one shared safe-back policy (`local-state first -> stack-first return -> route-specific fallback`) with approved no-history destinations for `/agenda`, `/agenda/evento/:slug`, `/parceiro/:slug`, `/mapa`, and `/mapa/poi`. | Prevents empty-root/dead-end back behavior without breaking source continuity for normal in-app navigation. | Sections `2.1`, `3` |
| `FCX-07` | Approved | Route-back governance is structural and centralized: route screens and shared back widgets must consume typed route-back policies/helpers instead of owning raw `pop/maybePop` policy, with explicit exemptions only for result-return/overlay flows. | Prevents new route surfaces from inventing private back behavior while keeping enforcement objective and analyzer-friendly. | Section `2.1` |
...
| `TODO-v1-canonical-back-navigation-governance-cutover.md` | Canonical route-back governance across tenant-public and tenant-admin | In progress | `2.1`, `7` | Structural rule promoted now: typed route-back helpers/handlers are mandatory for governed screens; semantic linting remains a later phase. |
