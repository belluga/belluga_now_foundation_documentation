...
| `EVS-ATT-02` | Approved | Presence confirmation and every participation relationship that acts on a scheduled experience are occurrence-scoped. `event_id` may remain parent/read context, but the relationship identity, uniqueness, counters, and supersession semantics use concrete `occurrence_id`. | Prevents one date of a multi-occurrence event from confirming, superseding, checking in, or reserving another date. | Sections `5.2`, `7` |
| `EVS-ATT-03` | Approved | Flutter client caches that expose confirmed occurrence ids are registered user-linked participation state and must participate in post-auth identity hydration after registered identity emission. | Prevents confirmed/attendance indicators from staying stale after OTP login while preserving `occurrence_id` as the only relationship identity. | Sections `5.1`, `7`; `foundation_documentation/modules/flutter_client_experience_module.md` `FCX-12` |
...
| `EVS-OCC-02` | Approved | `Event` + `EventOccurrence` consistency is transaction-owned domain behavior. Periodic global occurrence reconcile is not an approved steady-state mechanism; any remaining repair flow must be explicit/manual or event-targeted and must call the same canonical domain write path rather than rebuilding occurrences from a partial shape. | Prevents silent degradation of occurrence-scoped fields such as `programming_items` and own related profiles, eliminates wasteful full-tenant sweeps, and keeps aggregate consistency inside the same transaction-owned boundary as normal writes. | Sections `3.1`, `5.2`, `6` |
| `EVS-OCC-03` | Approved | Occurrence taxonomy terms are replacement overrides, not merged additions. If an occurrence owns taxonomy terms, effective filtering uses only those terms; if it owns none, filtering falls back to the parent event taxonomy terms. Occurrence terms may only use taxonomies allowed by the parent event type/category. | Keeps multi-date category/taxonomy filtering occurrence-first and prevents one date's sport/style from matching sibling dates. | Sections `5.1`, `5.2`, `5.4`, `7`; `TODO-store-release-agenda-card-polish-and-occurrence-taxonomy-overrides.md` |
| `EVS-PROG-01` | Approved | Programming items support optional same-day `end_time` in `HH:mm`; when present it must be later than `time` and is rendered as `time às end_time`. | Supports session-duration display without introducing cross-day programação semantics. | Sections `5.1`, `5.2`, `7`; `TODO-store-release-agenda-card-polish-and-occurrence-taxonomy-overrides.md` |
...
- Client confirmed-occurrence indicators consume user-scoped Participation/Attendance state, not Events core writes. Flutter repositories that cache confirmed occurrence ids must refresh after registered identity emission through the post-auth hydration contract before event detail, Agenda, or Home surfaces decide confirmed/empty indicators for the current user.
...
- Read payloads expose ordered `linked_account_profiles[]` projections suited to each surface. Event-wide summaries may aggregate explicit `event_parties` plus occurrence-owned profiles where the surface is event-wide, while occurrence-card/list summaries use the effective selected occurrence set (`event.event_parties + occurrence.event_parties`, deduplicated with event-level order first). Each summary entry is an account-profile input for UI grouping and contains enough identity data for direct navigation and taxonomy-aware cards (`id`, `slug`, `profile_type`, `display_name`, media fields, taxonomy term summaries).
- Event, Event Occurrence, venue, linked-profile, and compatibility artist taxonomy summaries expose display-ready taxonomy snapshots: `{type, value, name, taxonomy_name, label?}`. `name` is the canonical term display field, `taxonomy_name` is the group display field, and `label` is compatibility-only. Event Occurrence payloads also expose `own_taxonomy_terms` for occurrence-owned overrides; `taxonomy_terms` on an occurrence is the effective read snapshot.
...
- Multi-occurrence public detail keeps the event slug as route identity and carries selected occurrence identity through optional query metadata (`occurrence=<occurrence_id|occurrence_slug>`). The selected occurrence determines effective date/time, event-detail effective related profiles, and conditional `Programação` content.
- Detail occurrence arrays include one display-ready card input per occurrence/date. Each item exposes `occurrence_id`, `occurrence_slug`, `date_time_start`, `date_time_end`, `is_selected`, `own_linked_account_profiles`, `programming_count`, and `programming_items` when present. Occurrence card/list surfaces use the effective selected occurrence related-profile set (`event.event_parties + own_linked_account_profiles`, deduplicated); profiles from other occurrences must not leak into those cards.
- The occurrence-owned related-profile set is the canonical parent set for that date. Selected-detail effective related profiles are `event.event_parties + occurrence.event_parties`, deduplicated deterministically with event-level order first. Event-wide summaries use `event.event_parties + all occurrence.event_parties`, deduplicated with event-level profiles before occurrence order.
...
- Event and occurrence mirror consistency is transaction-owned. Normal create/update/delete/publication flows must not rely on a periodic global reconcile sweep to repair aggregate drift.
- Occurrence write payloads may include occurrence-owned `event_parties`, occurrence-owned `programming_items`, and occurrence-owned `taxonomy_terms`. Occurrence-level `location`/`place_ref` overrides are not part of the approved Store Release model; item-level programação locations use structured Account Profile/Map POI references instead. Omitted occurrence-owned fields on update preserve existing occurrence values; present fields replace their occurrence-scoped values in request order.
- `programming_items[].account_profile_ids` may reference only Account Profiles already present in the same occurrence-owned `event_parties` set. Admin UI may expose an inline shortcut that adds a profile to the occurrence first and then preselects it in the item, but the persisted contract remains occurrence-first rather than item-authored.
- `programming_items[].end_time` is optional, same-day, `HH:mm`, and must be later than `programming_items[].time` when present.
- `occurrences[].taxonomy_terms` may reference only taxonomy groups allowed by the selected event type/category. When present, those occurrence terms replace event-level terms for that occurrence's effective taxonomy; they are not merged with event-level taxonomy terms.
...
- Event `content` accepts only the approved canonical rich-text/HTML subset: `<p>`, `<br>`, `<h1-6>`, `<ul>`, `<ol>`, `<li>`, `<blockquote>`, `<strong>`, `<em>`, and `<s>`. Backend create/update sanitizes unsupported tags/attributes before persistence, preserves emojis as text, and frontend/editor flows must not imply unsupported markup will be preserved.
...
| `TODO-store-release-taxonomy-term-display-snapshots.md` | Taxonomy term display snapshots across Events/Event Occurrences and linked profile summaries | Promotion Lane | Sections 4, 5, 6 | Promotes display-ready taxonomy snapshots while preserving slug-pair filtering and occurrence-first read performance. |
| `TODO-store-release-event-multi-occurrence-ux-and-authoring-model.md` | Multi-occurrence selected-detail, occurrence-scoped related profiles, and occurrence-exclusive Programação with item-level location references | Promotion Lane | Sections 4, 5, 6 | Promotes selected-occurrence detail semantics and occurrence-owned payload extensions while preserving occurrence-first discovery and event slug identity. |
| `TODO-store-release-event-occurrence-transactional-consistency-and-reconcile-removal.md` | Remove periodic occurrence reconcile and make aggregate consistency transaction-owned | Promotion Lane | Sections 4, 5, 6 | Local implementation + Laravel validation complete on the current orchestrator branch: repair/manual occurrence resync now reuses the canonical aggregate write path, preserves occurrence-scoped fields, and downstream SR-D runtime recut is now promotion-ready. |
...
- `invite_and_social_loop_module.md` and any future Participation/Attendance module consume concrete occurrence identity for confirmations, reservations, check-ins, outcomes, and invite supersession. Event identity is parent context only for those relationships.
- `flutter_client_experience_module.md` owns client-side post-auth hydration for user-linked confirmed occurrence caches; Events remains the occurrence/read-contract authority and does not own authenticated user attendance state.
