...
- Detail occurrence arrays include one display-ready card input per occurrence/date. Each item exposes `occurrence_id`, `occurrence_slug`, `date_time_start`, `date_time_end`, `is_selected`, `own_linked_account_profiles`, `programming_count`, and `programming_items` when present. Occurrence card/list surfaces use `own_linked_account_profiles` only; event-shared profiles must not leak into occurrence cards.
