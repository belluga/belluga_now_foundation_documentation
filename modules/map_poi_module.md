...
- Warm permission entry uses an explicit boundary-result contract: `/location/permission` is pushed as a normal route, and the warm map entry flow handles `granted|continueWithoutLocation|cancelled` through the injected boundary callback. On `granted` or `continueWithoutLocation`, the flow replaces the permission boundary with `/mapa`; on `cancelled`, it pops back to the preserved predecessor route. The resulting stack must still behave as `home -> mapa`, not `home -> permission -> mapa`.
