...
* Invite flow now uses preview-first share-code bootstrap on `/invite?code=...` (same `InviteFlowRoute` surface): unauthenticated entry resolves `previewShareCode`, while authenticated entry resolves `acceptShareCode` and preserves attribution before rendering grouped invite feed UI.
...
* Platform deep-link readiness is versioned for guarappari: Android App Links + iOS Universal Links + `.well-known` artifacts, with test coverage validating manifest/entitlements/artifacts and canonical nginx serving declarations.
