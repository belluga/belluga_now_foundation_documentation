...
* Invite entry routes (`/invite`, `/convites`, `/convites/compartilhar`) are now explicitly guarded by `TenantRouteGuard` + `AuthRouteGuard`; auth redirect normalization preserves the original deep-link query (`code`) for login round-trip.
